...
int RGWHbacCtl::store_hbac(const DoutPrefixProvider *dpp, optional_yield y,
                           RGWHbacInfo &info, const PutParams &params) {
  return be_handler->call([&](RGWSI_MetaBackend_Handler::Op *op) {
    return svc.hbac->store_hbac_info(op->ctx(), info, params.objv_tracker,
                                     params.mtime, params.exclusive,
                                     params.attrs, y, dpp);
  });
}

int RGWHbacCtl::read_hbac(const DoutPrefixProvider *dpp, optional_yield y,
                          RGWHbacInfo &info, const GetParams &params) {
  return be_handler->call([&](RGWSI_MetaBackend_Handler::Op *op) {
    return svc.hbac->read_hbac_info(op->ctx(), info, params.objv_tracker,
...
int RGWHbacCtl::remove_hbac(const DoutPrefixProvider *dpp, optional_yield y,
  ...
  return be_handler->call([&](RGWSI_MetaBackend_Handler::Op *op) {
    return svc.hbac->remove_hbac_info(op->ctx(), info, params.objv_tracker, y,
                                      dpp);
  });
}
...
int RGWHbacCtl::read_user_hierarchy(const DoutPrefixProvider *dpp,
  ...
  return be_handler->call([&](RGWSI_MetaBackend_Handler::Op *op) {
    return svc.hbac->read_hierarchy_info(
        op->ctx(), hierarchy, params.objv_tracker, params.mtime,
        params.cache_info, params.attrs, y, dpp);
  });
}
...
public:
  ...
  string get_type() override { return "hbac"; }
...
RGWHbacCtl::RGWHbacCtl(RGWSI_Zone *zone_svc, RGWSI_HBAC_SObj *hbac_svc,
                       RGWHbacMetadataHandler *_hmhandler)
    : hmhandler(_hmhandler) {
  svc.zone = zone_svc;
  svc.hbac = hbac_svc;
