...
int RadosUser::list_buckets(const DoutPrefixProvider *dpp,
                            ...
                            bool need_stats, BucketList &result,
                            optional_yield y) {
  RGWUserBuckets ulist;
  bool is_truncated = false;

  int ret =
      store->ctl()->user->list_buckets(dpp, get_id(), marker, end_marker, max,
...
int RadosBucket::create(const DoutPrefixProvider *dpp,
                        const CreateParams &params, optional_yield y) {
...
int RadosUser::read_stats(const DoutPrefixProvider *dpp, optional_yield y,
...
int RadosUser::read_usage(const DoutPrefixProvider *dpp, uint64_t start_epoch,
...
int RadosUser::trim_usage(const DoutPrefixProvider *dpp, uint64_t start_epoch,
                          uint64_t end_epoch, optional_yield y) {
  std::string bucket_name;

  return store->getRados()->trim_usage(dpp, get_id(), bucket_name, start_epoch,
                                       end_epoch, y);
}
...
int RadosBucket::remove(const DoutPrefixProvider *dpp, bool delete_children,
                        optional_yield y) {
  int ret;
  ...
  ret =
      store->ctl()->bucket->sync_user_stats(dpp, info.owner, info, y, nullptr);
  if (ret < 0) {
    ldout(store->ctx(), 1)
  ...
  // they should be removed (note that any pending notifications on the bucket
  // are still going to be sent)
  ...
  ret = store->ctl()->bucket->unlink_bucket(info.owner, info.bucket, y, dpp,
                                            false);
  if (ret < 0) {
...
int RadosBucket::remove_bypass_gc(int concurrent_max,
  ...
  ret = read_stats(dpp, index, RGW_NO_SHARD, &bucket_ver, &master_ver, stats,
                   NULL);
  if (ret < 0)
    return ret;
  ...
  while (results.is_truncated) {
    ...
    for (; it != results.objs.end(); ++it) {
      ...
      ret = store->getRados()->get_obj_state(dpp, &obj_ctx, get_info(), obj,
                                             &astate, &amanifest, false, y);
      if (ret == -ENOENT) {
      ...
      if (amanifest) {
        ...
        store->get_raw_obj(manifest.get_head_placement_rule(), head_obj,
                           &raw_head_obj);
        ...
        ret = store->getRados()->delete_obj_aio(dpp, head_obj, get_info(),
                                                astate, handles,
...
std::string RadosHbac::to_str() { return info.make_response(); }
...
int RadosHbac::remove_hbac(const DoutPrefixProvider *dpp, optional_yield y) {
...
int RadosBucket::unlink(const DoutPrefixProvider *dpp, const rgw_user &owner,
                        optional_yield y, bool update_entrypoint) {
  return store->ctl()->bucket->unlink_bucket(owner, info.bucket, y, dpp,
                                             update_entrypoint);
}

int RadosBucket::chown(const DoutPrefixProvider *dpp, const rgw_user &new_owner,
                       optional_yield y) {
...
int RadosBucket::put_info(const DoutPrefixProvider *dpp, bool exclusive,
  ...
  mtime = _mtime;
  return store->getRados()->put_bucket_instance_info(info, exclusive, mtime,
                                                     &attrs, dpp, y);
}
...
int RadosBucket::check_quota(const DoutPrefixProvider *dpp, RGWQuota &quota,
...
int RadosBucket::merge_and_store_attrs(const DoutPrefixProvider *dpp,
  ...
  return store->ctl()->bucket->set_bucket_instance_attrs(
      get_info(), new_attrs, &get_info().objv_tracker, y, dpp);
}

int RadosBucket::try_refresh_info(const DoutPrefixProvider *dpp,
                                  ceph::real_time *pmtime, optional_yield y) {
  return store->getRados()->try_refresh_bucket_info(info, pmtime, dpp, y,
                                                    &attrs);
}

int RadosBucket::read_usage(const DoutPrefixProvider *dpp, uint64_t start_epoch,
                            uint64_t end_epoch, uint32_t max_entries,
                            bool *is_truncated, RGWUsageIter &usage_iter,
                            map<rgw_user_bucket, rgw_usage_log_entry> &usage) {
...
int RadosBucket::trim_usage(const DoutPrefixProvider *dpp, uint64_t start_epoch,
                            uint64_t end_epoch, optional_yield y) {
  return store->getRados()->trim_usage(dpp, info.owner, get_name(), start_epoch,
                                       end_epoch, y);
}

int RadosBucket::remove_objs_from_index(
    const DoutPrefixProvider *dpp,
    std::list<rgw_obj_index_key> &objs_to_unlink) {
  return store->getRados()->remove_objs_from_index(dpp, info, objs_to_unlink);
}

int RadosBucket::check_index(
    const DoutPrefixProvider *dpp,
...
int RadosBucket::list(const DoutPrefixProvider *dpp, ListParams &params,
  ...
  int ret =
      list_op.list_objects(dpp, max, &results.objs, &results.common_prefixes,
...
int RadosBucket::list_multiparts(
  ...
  if (!results.objs.empty()) {
    for (const rgw_bucket_dir_entry &dentry : results.objs) {
      ...
      const ACLOwner owner{.id = rgw_user(dentry.meta.owner),
...
int RadosBucket::abort_multiparts(const DoutPrefixProvider *dpp,
  ...
  do {
    ret = list_multiparts(dpp, empty_prefix, marker, empty_delim, max, uploads,
                          nullptr, &is_truncated, y);
    if (ret < 0) {
...
int RadosStore::load_hbac(const DoutPrefixProvider *dpp, rgw_hbac_info info,
...
int RadosStore::remove_hbac(const DoutPrefixProvider *dpp, rgw_hbac_info info,
                            std::unique_ptr<rgw::sal::Hbac> *hbac,
                            optional_yield y) {
  *hbac = std::make_unique<RadosHbac>(this, info);
  return (*hbac)->remove_hbac(dpp, y);
}
...
bool RadosStore::is_meta_master() { return svc()->zone->is_meta_master(); }
...
RadosStore::get_notification(rgw::sal::Object *obj, rgw::sal::Object *src_obj,
...
int RadosStore::register_to_service_map(
...
int RadosStore::meta_list_keys_next(const DoutPrefixProvider *dpp, void *handle,
                                    int max, list<std::string> &keys,
                                    bool *truncated) {
...
RadosStore::get_role(std::string name, std::string tenant, std::string path,
                     std::string trust_policy,
                     std::string max_session_duration_str,
                     std::multimap<std::string, std::string> tags) {
  return std::make_unique<RadosRole>(this, name, tenant, path, trust_policy,
...
int RadosStore::get_roles(const DoutPrefixProvider *dpp, optional_yield y,
  ...
  do {
    ...
    int r = rados->list_raw_objects(dpp, pool, prefix, 1000, ctx, oids,
                                    &is_truncated);
    if (r < 0) {
      ldpp_dout(dpp, 0) << "ERROR: listing filtered objects failed: " << prefix
  ...
  for (const auto &it : result) {
    ...
    /*Make sure that prefix is part of path (False results could've been
      returned) because of the role info oid + id appended to the path)*/
...
int RadosStore::get_oidc_providers(
  ...
  do {
    ...
    for (const auto &iter : oids) {
      ...
      r = rgw_get_system_obj(svc()->sysobj, pool, iter, bl, nullptr, nullptr, y,
                             dpp);
      if (r < 0) {
        return r;
      }
...
RadosStore::get_append_writer(const DoutPrefixProvider *dpp, optional_yield y,
...
std::unique_ptr<Writer> RadosStore::get_atomic_writer(
...
int RadosObject::set_obj_attrs(const DoutPrefixProvider *dpp, Attrs *setattrs,
                               Attrs *delattrs, optional_yield y) {
  Attrs empty;
  return store->getRados()->set_attrs(dpp, rados_ctx, bucket->get_info(),
                                      get_obj(), setattrs ? *setattrs : empty,
                                      delattrs ? delattrs : nullptr, y);
}

int RadosObject::get_obj_attrs(optional_yield y, const DoutPrefixProvider *dpp,
                               rgw_obj *target_obj) {
  RGWRados::Object op_target(store->getRados(), bucket->get_info(), *rados_ctx,
                             get_obj());
...
int RadosObject::modify_obj_attrs(const char *attr_name, bufferlist &attr_val,
                                  optional_yield y,
                                  const DoutPrefixProvider *dpp) {
...
void RadosObject::get_raw_obj(rgw_raw_obj *raw_obj) {
  store->getRados()->obj_to_raw((bucket->get_info()).placement_rule, get_obj(),
                                raw_obj);
}
...
int RadosObject::transition(Bucket *bucket,
  ...
  return store->getRados()->transition_obj(*rados_ctx, bucket->get_info(),
                                           get_obj(), placement_rule, mtime,
                                           olh_epoch, dpp, y);
}

int RadosObject::transition_to_cloud(
  ...
  S3RESTConn conn(cct, id, {endpoint}, key, zonegroup.get_id(), region,
                  host_style);

  RGWLCCloudTierCtx tier_ctx(cct, dpp, o, store, bucket->get_info(), this, conn,
                             bucket_name,
  ...
  if (update_object) {
    ...
    ret = write_cloud_tier(dpp, y, tier_ctx.o.versioned_epoch, tier,
                           tier_ctx.is_multipart_upload, target_placement,
                           tier_ctx.obj);
  }
...
int RadosObject::write_cloud_tier(const DoutPrefixProvider *dpp,
                                  optional_yield y, uint64_t olh_epoch,
                                  PlacementTier *tier, bool is_multipart_upload,
                                  rgw_placement_rule &target_placement,
                                  Object *head_obj) {
  rgw::sal::RadosPlacementTier *rtier =
      static_cast<rgw::sal::RadosPlacementTier *>(tier);
...
RadosObject::RadosReadOp::RadosReadOp(RadosObject *_source, RGWObjectCtx *_octx)
    : source(_source), octx(_octx),
      op_target(_source->store->getRados(), _source->get_bucket()->get_info(),
                *static_cast<RGWObjectCtx *>(octx), _source->get_obj()),
      parent_op(&op_target) {}
...
int RadosObject::RadosReadOp::read(int64_t ofs, int64_t end, bufferlist &bl,
                                   optional_yield y,
                                   const DoutPrefixProvider *dpp) {
...
RadosObject::RadosDeleteOp::RadosDeleteOp(RadosObject *_source)
    : source(_source),
      op_target(_source->store->getRados(), _source->get_bucket()->get_info(),
                _source->get_ctx(), _source->get_obj()),
      parent_op(&op_target) {}
...
int RadosObject::delete_object(const DoutPrefixProvider *dpp, optional_yield y,
                               bool prevent_versioning) {
  RGWRados::Object del_target(store->getRados(), bucket->get_info(), *rados_ctx,
                              get_obj());
...
int RadosObject::copy_object(
  ...
  return store->getRados()->copy_obj(
      *rados_ctx, user->get_id(), info, source_zone, dest_object->get_obj(),
      get_obj(), dest_bucket->get_info(), src_bucket->get_info(),
      dest_placement, src_mtime, mtime, mod_ptr, unmod_ptr, high_precision_time,
      if_match, if_nomatch, static_cast<RGWRados::AttrsMod>(attrs_mod),
      copy_if_newer, attrs, category, olh_epoch,
      (delete_at ? *delete_at : real_time()), version_id, tag, etag,
...
int RadosObject::swift_versioning_restore(bool &restored,
  ...
  return store->getRados()->swift_versioning_restore(
      *rados_ctx, bucket->get_owner(), bucket->get_info(), obj, restored, dpp,
      y);
}
...
int RadosObject::swift_versioning_copy(const DoutPrefixProvider *dpp,
  ...
  return store->getRados()->swift_versioning_copy(
      *rados_ctx, bucket->get_info().owner, bucket->get_info(), get_obj(), dpp,
      y);
}

int RadosMultipartUpload::cleanup_part_history(
    const DoutPrefixProvider *dpp, optional_yield y, RadosMultipartPart *part,
    list<rgw_obj_index_key> &remove_objs) {
  cls_rgw_obj_chain chain;
  for (auto &ppfx : part->get_past_prefixes()) {
    rgw_obj past_obj;
    past_obj.init_ns(bucket->get_key(),
                     ppfx + "." + std::to_string(part->info.num), mp_ns);
  ...
  if (store->getRados()->get_gc() == nullptr) {
    // Delete objects inline if gc hasn't been initialised (in case when bypass
    // gc is specified)
  ...
  do {
    ...
    for (auto part_it = parts.begin(); part_it != parts.end(); ++part_it) {
      ...
      if (obj_part->info.manifest.empty()) {
        ...
        store->getRados()->update_gc_chain(dpp, target, obj_part->info.manifest,
                                           &chain);
...
int RadosMultipartUpload::init(const DoutPrefixProvider *dpp, optional_yield y,
                               ACLOwner &owner,
  ...
  do {
    ...
    RGWRados::Object op_target(store->getRados(), obj->get_bucket()->get_info(),
                               obj_ctx, obj->get_obj());
    RGWRados::Object::Write obj_op(&op_target);
  ...
  for (i = 0, iter = parts_map.begin();
    ...
    if (sorted_omap) {
      if (part->info.num != expected_next) {
         ...
         */
        return list_parts(dpp, cct, num_parts, marker, next_marker, truncated,
                          y, true);
      }
      expected_next++;
    }
...
int RadosMultipartUpload::complete(
    ...
    off_t &ofs, std::string &tag, ACLOwner &owner, uint64_t olh_epoch,
...
MPRadosSerializer::MPRadosSerializer(const DoutPrefixProvider *dpp,
...
LCRadosSerializer::LCRadosSerializer(RadosStore *store, const std::string &_oid,
                                     const std::string &lock_name,
...
int RadosLifecycle::get_entry(const std::string &oid, const std::string &marker,
  ...
  int ret = cls_rgw_lc_get_entry(*store->getRados()->get_lc_pool_ctx(), oid,
                                 marker, cls_entry);
  if (ret)
    return ret;
...
int RadosLifecycle::get_next_entry(const std::string &oid,
                                   ...
                                   std::unique_ptr<LCEntry> *entry) {
  cls_rgw_lc_entry cls_entry;
  int ret = cls_rgw_lc_get_next_entry(*store->getRados()->get_lc_pool_ctx(),
                                      oid, marker, cls_entry);
...
int RadosLifecycle::list_entries(
  ...
  int ret = cls_rgw_lc_list(*store->getRados()->get_lc_pool_ctx(), oid, marker,
  ...
  for (auto &entry : cls_entries) {
    entries.push_back(std::make_unique<StoreLCEntry>(
        entry.bucket, oid, entry.start_time, entry.status));
  }
...
int RadosAtomicWriter::complete(
...
int RadosAppendWriter::complete(
    size_t accounted_size, const std::string &etag, ceph::real_time *mtime,
    ceph::real_time set_mtime, std::map<std::string, bufferlist> &attrs,
    ceph::real_time delete_at, const char *if_match, const char *if_nomatch,
    const std::string *user_data, rgw_zone_set *zones_trace, bool *canceled,
...
void RadosZoneGroup::get_placement_target_names(
...
RadosLuaManager::RadosLuaManager(RadosStore *_s,
    ...
    : StoreLuaManager(_luarocks_path), store(_s),
      pool((store->svc() && store->svc()->zone)
               ? store->svc()->zone->get_zone_params().log_pool
               : rgw_pool()),
      ioctx(*store->getRados()->get_lc_pool_ctx()), packages_watcher(this) {}

int RadosLuaManager::get_script(const DoutPrefixProvider *dpp, optional_yield y,
  ...
  int r = rgw_get_system_obj(store->svc()->sysobj, pool, key, bl, nullptr,
                             nullptr, y, dpp);
  if (r < 0) {
    return r;
  }
...
int RadosLuaManager::put_script(const DoutPrefixProvider *dpp, optional_yield y,
  ...
  int r = rgw_put_system_obj(dpp, store->svc()->sysobj, pool, key, bl, false,
                             nullptr, real_time(), y);
  if (r < 0) {
    return r;
  }
...
int RadosLuaManager::del_script(const DoutPrefixProvider *dpp, optional_yield y,
                                const std::string &key) {
  if (pool.empty()) {
...
int RadosLuaManager::add_package(const DoutPrefixProvider *dpp,
  ...
  return rgw_rados_operate(dpp, ioctx, PACKAGE_LIST_OBJECT_NAME, &op, y);
}
...
int RadosLuaManager::remove_package(const DoutPrefixProvider *dpp,
  ...
  if (pos != package_name.npos) {
    ...
    auto ret = rgw_rados_operate(dpp, ioctx, PACKAGE_LIST_OBJECT_NAME, &op, y);
    if (ret < 0) {
      return ret;
    }
    return 0;
  ...
  for (const auto &package : packages) {
    ...
    if (package_no_version.compare(package_name) == 0) {
      ...
      ret = rgw_rados_operate(dpp, ioctx, PACKAGE_LIST_OBJECT_NAME, &op, y);
      if (ret < 0) {
        return ret;
      }
    }
...
int RadosLuaManager::list_packages(const DoutPrefixProvider *dpp,
  ...
  while (more) {
    ...
    const auto ret = rgw_rados_operate(dpp, ioctx, PACKAGE_LIST_OBJECT_NAME,
                                       &op, nullptr, y);
...
int RadosLuaManager::watch_reload(const DoutPrefixProvider *dpp) {
  ...
  op.create(false);
  auto r =
      rgw_rados_operate(dpp, ioctx, PACKAGE_LIST_OBJECT_NAME, &op, null_yield);
  if (r < 0) {
...
void RadosLuaManager::ack_reload(const DoutPrefixProvider *dpp,
...
void RadosLuaManager::handle_reload_notify(const DoutPrefixProvider *dpp,
...
int RadosLuaManager::reload_packages(const DoutPrefixProvider *dpp,
                                     optional_yield y) {
  if (!ioctx.is_valid()) {
    ldpp_dout(dpp, 10)
  ...
  auto r = rgw_rados_notify(dpp, ioctx, PACKAGE_LIST_OBJECT_NAME, empty_bl,
...
void RadosLuaManager::PackagesWatcher::handle_notify(uint64_t notify_id,
                                                     uint64_t cookie,
...
int RadosOIDCProvider::store_url(const DoutPrefixProvider *dpp,
  ...
  return rgw_put_system_obj(dpp, sysobj,
                            store->svc()->zone->get_zone_params().oidc_pool,
                            oid, bl, exclusive, nullptr, real_time(), y);
}
...
int RadosOIDCProvider::delete_obj(const DoutPrefixProvider *dpp,
  ...
  if (this->tenant != tenant) {
    ldpp_dout(dpp, 0) << "ERROR: tenant in arn doesn't match that of user "
...
int RadosRole::store_info(const DoutPrefixProvider *dpp, bool exclusive,
  ...
  if (!this->info.tags.empty()) {
    ...
    std::unique_ptr<RGWSI_MetaBackend::Context> ctx(
        store->svc()->role->svc.meta_be->alloc_ctx());
...
int RadosRole::store_name(const DoutPrefixProvider *dpp, bool exclusive,
  ...
  return rgw_put_system_obj(
      dpp, sysobj, store->svc()->zone->get_zone_params().roles_pool, oid, bl,
      exclusive, &info.objv_tracker, real_time(), y);
}

int RadosRole::store_path(const DoutPrefixProvider *dpp, bool exclusive,
                          optional_yield y) {
  auto sysobj = store->svc()->sysobj;
  std::string oid = info.tenant + get_path_oid_prefix() + info.path +
...
int RadosRole::read_id(const DoutPrefixProvider *dpp,
  ...
  try {
    ...
    ldpp_dout(dpp, 0) << "ERROR: failed to decode role from Role pool: "
                      << role_name << dendl;
    return -EIO;
  }
...
int RadosRole::read_name(const DoutPrefixProvider *dpp, optional_yield y) {
  ...
  int ret = rgw_get_system_obj(sysobj,
                               ...
                               oid, bl, nullptr, nullptr, y, dpp);
  if (ret < 0) {
    ldpp_dout(dpp, 0) << "ERROR: failed reading role name from Role pool: "
  ...
  try {
    ...
    ldpp_dout(dpp, 0) << "ERROR: failed to decode role name from Role pool: "
                      << info.name << dendl;
    return -EIO;
  }
...
int RadosRole::read_info(const DoutPrefixProvider *dpp, optional_yield y) {
  ...
  std::unique_ptr<RGWSI_MetaBackend::Context> ctx(
      store->svc()->role->svc.meta_be->alloc_ctx());
  ctx->init(store->svc()->role->get_be_handler());
  int ret = store->svc()->role->svc.meta_be->get(
      ctx.get(), oid, params, &info.objv_tracker, y, dpp, true);
  if (ret < 0) {
    ldpp_dout(dpp, 0) << "ERROR: failed reading role info from Role pool: "
  ...
  try {
    ...
    ldpp_dout(dpp, 0) << "ERROR: failed to decode role info from Role pool: "
                      << info.id << dendl;
    return -EIO;
  }
...
int RadosRole::create(const DoutPrefixProvider *dpp, bool exclusive,
  ...
  if (exclusive && ret == 0) {
    ldpp_dout(dpp, 0) << "ERROR: name " << info.name
                      << " already in use for role id " << info.id << dendl;
    return -EEXIST;
  ...
  if (ret < 0) {
    ldpp_dout(dpp, 0) << "ERROR:  storing role info in Role pool: " << info.id
                      << ": " << cpp_strerror(-ret) << dendl;
    return ret;
  }
  ...
  if (ret < 0) {
    ldpp_dout(dpp, 0) << "ERROR: storing role name in Role pool: " << info.name
                      << ": " << cpp_strerror(-ret) << dendl;

    // Delete the role info that was stored in the previous call
    std::string oid = get_info_oid_prefix() + info.id;
    int info_ret =
        rgw_delete_system_obj(dpp, store->svc()->sysobj, pool, oid, nullptr, y);
    if (info_ret < 0) {
  ...
  if (ret < 0) {
    ldpp_dout(dpp, 0) << "ERROR: storing role path in Role pool: " << info.path
                      << ": " << cpp_strerror(-ret) << dendl;
    // Delete the role info that was stored in the previous call
    std::string oid = get_info_oid_prefix() + info.id;
    int info_ret =
    ...
    if (name_ret < 0) {
      ldpp_dout(dpp, 0) << "ERROR: cleanup of role name from Role pool: "
                        << info.name << ": " << cpp_strerror(-name_ret)
                        << dendl;
    }
    return ret;
  }
...
int RadosRole::delete_obj(const DoutPrefixProvider *dpp, optional_yield y) {
  ...
  ret = rgw_delete_system_obj(dpp, store->svc()->sysobj, pool, oid, nullptr, y);
  if (ret < 0) {
    ldpp_dout(dpp, 0) << "ERROR: deleting role id from Role pool: " << info.id
                      << ": " << cpp_strerror(-ret) << dendl;
  }
  ...
  // Delete path
  oid = info.tenant + get_path_oid_prefix() + info.path +
        get_info_oid_prefix() + info.id;
  ret = rgw_delete_system_obj(dpp, store->svc()->sysobj, pool, oid, nullptr, y);
  if (ret < 0) {
