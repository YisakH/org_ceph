...
public:
  ...
  get_lua_manager(const std::string &luarocks_path) override;
  ...
  get_compression_type(const rgw_placement_rule &rule) override;
  ...
  int delete_raw_obj(const DoutPrefixProvider *dpp, const rgw_raw_obj &obj,
                     optional_yield y);
  void get_raw_obj(const rgw_placement_rule &placement_rule, const rgw_obj &obj,
                   rgw_raw_obj *raw_obj);
  int get_raw_chunk_size(const DoutPrefixProvider *dpp, const rgw_raw_obj &obj,
                         uint64_t *chunk_size);
  ...
  int load_hbac(const DoutPrefixProvider *dpp, rgw_hbac_info info,
                std::unique_ptr<rgw::sal::Hbac> *hbac,
                optional_yield y) override;
  int store_hbac(const DoutPrefixProvider *dpp, rgw_hbac_info info,
                 std::unique_ptr<rgw::sal::Hbac> *hbac,
                 optional_yield y) override;
...
public:
  ...
  virtual int
  copy_object(User *user, req_info *info, const rgw_zone_id &source_zone,
  ...
  int write_cloud_tier(const DoutPrefixProvider *dpp, optional_yield y,
                       uint64_t olh_epoch, rgw::sal::PlacementTier *tier,
                       bool is_multipart_upload,
                       rgw_placement_rule &target_placement, Object *head_obj);
  RGWObjManifest *get_manifest() { return manifest; }
  RGWObjectCtx &get_ctx() { return *rados_ctx; }

private:
  int read_attrs(const DoutPrefixProvider *dpp, RGWRados::Object::Read &read_op,
                 optional_yield y, rgw_obj *target_obj = nullptr);
};

class RadosHbac : public Hbac {
private:
  RadosStore *store;
  RGWHbacInfo info;
...
public:
  ...
  RadosHbac(RadosStore *_st, const rgw_hbac_info &_h) : store(_st), info(_h){};
  ~RadosHbac() = default;
...
public:
  RadosBucket(RadosStore *_st) : store(_st), acls() {}
...
private:
  int link(const DoutPrefixProvider *dpp, const rgw_user &new_owner,
           optional_yield y, bool update_entrypoint = true,
           RGWObjVersionTracker *objv = nullptr);
  int unlink(const DoutPrefixProvider *dpp, const rgw_user &owner,
             optional_yield y, bool update_entrypoint = true);
...
protected:
  int cleanup_part_history(const DoutPrefixProvider *dpp, optional_yield y,
...
public:
  MPRadosSerializer(const DoutPrefixProvider *dpp, RadosStore *store,
                    RadosObject *obj, const std::string &lock_name);
...
public:
  ...
  virtual int
  list_entries(const std::string &oid, const std::string &marker,
...
public:
  RadosNotification(const DoutPrefixProvider *_dpp, RadosStore *_store,
                    Object *_obj, Object *_src_obj, req_state *_s,
                    rgw::notify::EventType _type, optional_yield y,
                    const std::string *object_name)
      : StoreNotification(_obj, _src_obj, _type), store(_store),
        res(_dpp, _store, _s, _obj, _src_obj, object_name, y) {}

  RadosNotification(const DoutPrefixProvider *_dpp, RadosStore *_store,
                    Object *_obj, Object *_src_obj,
                    rgw::notify::EventType _type, rgw::sal::Bucket *_bucket,
                    std::string &_user_id, std::string &_user_tenant,
                    std::string &_req_id, optional_yield y)
      : StoreNotification(_obj, _src_obj, _type), store(_store),
        res(_dpp, _store, _obj, _src_obj, _bucket, _user_id, _user_tenant,
            _req_id, y) {}
...
public:
  RadosAtomicWriter(const DoutPrefixProvider *dpp, optional_yield y,
                    RGWBucketInfo &bucket_info, RGWObjectCtx &obj_ctx,
                    const rgw_obj &obj, RadosStore *_store,
                    std::unique_ptr<Aio> _aio, const rgw_user &owner,
                    const rgw_placement_rule *ptail_placement_rule,
                    uint64_t olh_epoch, const std::string &unique_tag)
      : StoreWriter(dpp, y), store(_store), aio(std::move(_aio)),
        obj_ctx(obj_ctx),
        processor(&*aio, store->getRados(), bucket_info, ptail_placement_rule,
                  owner, obj_ctx, obj, olh_epoch, unique_tag, dpp, y) {}
...
public:
  RadosAppendWriter(const DoutPrefixProvider *dpp, optional_yield y,
                    RGWBucketInfo &bucket_info, RGWObjectCtx &obj_ctx,
                    const rgw_obj &obj, RadosStore *_store,
                    std::unique_ptr<Aio> _aio, const rgw_user &owner,
                    const rgw_placement_rule *ptail_placement_rule,
      ...
      : StoreWriter(dpp, y), store(_store), aio(std::move(_aio)),
        obj_ctx(obj_ctx),
        processor(&*aio, store->getRados(), bucket_info, ptail_placement_rule,
                  owner, obj_ctx, obj, unique_tag, position, cur_accounted_size,
                  dpp, y) {}
...
public:
  RadosMultipartWriter(const DoutPrefixProvider *dpp, optional_yield y,
                       const std::string &upload_id, RGWBucketInfo &bucket_info,
      ...
      : StoreWriter(dpp, y), store(_store), aio(std::move(_aio)),
        obj_ctx(obj_ctx), processor(&*aio, store->getRados(), bucket_info,
                                    ptail_placement_rule, owner, obj_ctx, obj,
                                    upload_id, part_num, part_num_str, dpp, y) {
  }
...
class RadosLuaManager : public StoreLuaManager {
  ...
  public:
    PackagesWatcher(RadosLuaManager *_parent) : parent(_parent) {}
  ...
  void ack_reload(const DoutPrefixProvider *dpp, uint64_t notify_id,
                  uint64_t cookie, int reload_status);
  void handle_reload_notify(const DoutPrefixProvider *dpp, optional_yield y,
...
public:
  ...
  virtual int read_url(const DoutPrefixProvider *dpp, const std::string &url,
                       const std::string &tenant, optional_yield y) override;
  virtual int delete_obj(const DoutPrefixProvider *dpp,
                         optional_yield y) override;
...
public:
  RadosRole(RadosStore *_store, std::string name, std::string tenant,
  ...
  virtual int store_info(const DoutPrefixProvider *dpp, bool exclusive,
                         optional_yield y) override;
  virtual int store_name(const DoutPrefixProvider *dpp, bool exclusive,
                         optional_yield y) override;
  virtual int store_path(const DoutPrefixProvider *dpp, bool exclusive,
                         optional_yield y) override;
  ...
  virtual int create(const DoutPrefixProvider *dpp, bool exclusive,
                     const std::string &role_id, optional_yield y) override;
  virtual int delete_obj(const DoutPrefixProvider *dpp,
                         optional_yield y) override;
};
} // namespace sal
