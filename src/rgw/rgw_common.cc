...
rgw_http_errors rgw_http_s3_errors({
    ...
    {RGW_HBAC_KEY_NOT_FOUND, {404, "HBACKeyNotFound"}},
...
rgw_http_errors rgw_http_swift_errors({
     ...
     * procedures also for ERR_ZERO_IN_URL. This make a problem as the
     * validation is performed very early, even before setting the
     * req_state::proto_flags. */
...
bool rgw_err::is_clear() const { return (http_ret == 200); }

bool rgw_err::is_err() const { return !(http_ret >= 200 && http_ret <= 399); }
...
static string get_abs_path(const string &request_uri) {
  const static string ABS_PREFIXES[] = {"http://", "https://", "ws://",
                                        "wss://"};
  bool isAbs = false;
  for (int i = 0; i < 4; ++i) {
...
req_state::req_state(CephContext *_cct, const RGWProcessEnv &penv, RGWEnv *e,
                     uint64_t id)
...
void req_info::init_meta_info(const DoutPrefixProvider *dpp,
                              bool *found_bad_meta, const int prot_flags) {
  x_meta_map.clear();
  ...
  for (const auto &kv : env->get_map()) {
    ...
    for (int prefix_num = 0; (prefix = meta_prefixes[prefix_num].str) != NULL;
      ...
      if (strncmp(p, prefix, len) == 0) {
        ...
        string name_low =
            lowercase_dash_http_attr(string(meta_prefixes[0].str + 5) + name,
        ...
        if (strncmp(name_low.c_str(), "x-amz-server-side-encryption", 20) ==
            0) {
...
void rgw_add_amz_meta_header(meta_map_t &x_meta_map, const std::string &k,
                             const std::string &v) {
...
bool rgw_set_amz_meta_header(meta_map_t &x_meta_map, const std::string &k,
...
int parse_key_value(const string &in_str, const char *delim, string &key,
                    string &val) {
  if (delim == NULL)
    return -EINVAL;
 ...
 */
void calc_hmac_sha1(const char *key, int key_len, const char *msg, int msg_len,
                    char *dest)
 ...
 */
void calc_hmac_sha256(const char *key, int key_len, const char *msg,
                      int msg_len, char *dest) {
...
SHA256 *calc_hash_sha256_open_stream() { return new SHA256; }
...
void RGWHTTPArgs::append(const string &name, const string &val) {
  ...
  // when sub_resources exclusive by object are added, please remember to update
  // obj_sub_resource in RGWHTTPArgs::exist_obj_excl_sub_resource().
  ...
  } else if ((name.compare("subuser") == 0) || (name.compare("key") == 0) ||
...
bool rgw_transport_is_secure(CephContext *cct, const RGWEnv &env) {
  ...
  // Forwarded: by=<identifier>; for=<identifier>; host=<host>;
  // proto=<http|https>
...
struct perm_state_from_req_state : public perm_state_base {
  ...
  perm_state_from_req_state(req_state *const _s)
      : perm_state_base(
            _s->cct, _s->env, _s->auth.identity.get(),
            _s->bucket.get() ? _s->bucket->get_info() : RGWBucketInfo(),
            _s->perm_mask, _s->defer_to_bucket_acls, _s->bucket_access_conf),
        s(_s) {}
...
Effect eval_or_pass(
    const DoutPrefixProvider *dpp, const boost::optional<Policy> &policy,
...
bool verify_user_permission(const DoutPrefixProvider *dpp,
  ...
  if (!session_policies.empty()) {
    auto session_policy_res = eval_identity_or_session_policies(
        dpp, session_policies, s->env, op, res);
...
bool verify_user_permission(const DoutPrefixProvider *dpp, req_state *const s,
  ...
  return verify_user_permission(dpp, &ps, s->user_acl, s->iam_user_policies,
                                s->session_policies, res, op, mandatory_policy);
}
...
bool verify_bucket_permission(const DoutPrefixProvider *dpp,
  ...
  auto identity_policy_res = eval_identity_or_session_policies(
      dpp, identity_policies, s->env, op, ARN(bucket));
  if (identity_policy_res == Effect::Deny)
    return false;
  ...
  auto r = eval_or_pass(dpp, bucket_policy, s->env, *s->identity, op,
                        ARN(bucket), princ_type);
  if (r == Effect::Deny)
    return false;

  // Take into account session policies, if the identity making a request is a
  // role
  if (!session_policies.empty()) {
    auto session_policy_res = eval_identity_or_session_policies(
        dpp, session_policies, s->env, op, ARN(bucket));
    if (session_policy_res == Effect::Deny) {
      return false;
    }
    if (princ_type == rgw::IAM::PolicyPrincipal::Role) {
      // Intersection of session policy and identity policy plus intersection of
      // session policy and bucket policy
      ...
      if ((session_policy_res == Effect::Allow &&
        ...
        return true;
               ...
               rgw::IAM::PolicyPrincipal::Other) { // there was no match in the
                                                   // bucket policy
      if (session_policy_res == Effect::Allow &&
          identity_policy_res == Effect::Allow)
        return true;
...
bool verify_bucket_permission(const DoutPrefixProvider *dpp, req_state *const s,
                              const rgw_bucket &bucket,
                              const RGWAccessControlPolicy &user_acl,
                              const RGWAccessControlPolicy &bucket_acl,
                              const boost::optional<Policy> &bucket_policy,
                              ...
                              const uint64_t op) {
  perm_state_from_req_state ps(s);
  return verify_bucket_permission(dpp, &ps, bucket, user_acl, bucket_acl,
                                  bucket_policy, user_policies,
                                  session_policies, op);
}

bool verify_bucket_permission_no_policy(
    const DoutPrefixProvider *dpp, struct perm_state_base *const s,
    const RGWAccessControlPolicy &user_acl,
    const RGWAccessControlPolicy &bucket_acl, const int perm) {
  ...
  if (bucket_acl.verify_permission(
          dpp, *s->identity, perm, perm, s->get_referer(),
...
bool verify_bucket_permission_no_policy(
    const DoutPrefixProvider *dpp, req_state *const s,
    const RGWAccessControlPolicy &user_acl,
    const RGWAccessControlPolicy &bucket_acl, const int perm) {
  perm_state_from_req_state ps(s);
  return verify_bucket_permission_no_policy(dpp, &ps, user_acl, bucket_acl,
                                            perm);
}
...
bool verify_bucket_permission_no_policy(const DoutPrefixProvider *dpp,
  ...
  return verify_bucket_permission_no_policy(dpp, &ps, s->user_acl,
                                            s->bucket_acl, perm);
}
...
bool verify_bucket_permission(const DoutPrefixProvider *dpp, req_state *const s,
  ...
  return verify_bucket_permission(
      dpp, &ps, s->bucket->get_key(), s->user_acl, s->bucket_acl, s->iam_policy,
      s->iam_user_policies, s->session_policies, op);
}

// Authorize anyone permitted by the bucket policy, identity policies, session
// policies and the bucket owner unless explicitly denied by the policy.

int verify_bucket_owner_or_policy(req_state *const s, const uint64_t op) {
  auto identity_policy_res = eval_identity_or_session_policies(
      s, s->iam_user_policies, s->env, op, ARN(s->bucket->get_key()));
  if (identity_policy_res == Effect::Deny) {
    return -EACCES;
  }

  rgw::IAM::PolicyPrincipal princ_type = rgw::IAM::PolicyPrincipal::Other;
  auto e = eval_or_pass(s, s->iam_policy, s->env, *s->auth.identity, op,
                        ARN(s->bucket->get_key()), princ_type);
  if (e == Effect::Deny) {
    return -EACCES;
  }
  ...
  if (!s->session_policies.empty()) {
    auto session_policy_res = eval_identity_or_session_policies(
        s, s->session_policies, s->env, op, ARN(s->bucket->get_key()));
    if (session_policy_res == Effect::Deny) {
      return -EACCES;
    }
    if (princ_type == rgw::IAM::PolicyPrincipal::Role) {
...
static inline bool check_deferred_bucket_perms(
    const DoutPrefixProvider *dpp, struct perm_state_base *const s,
    const rgw_bucket &bucket, const RGWAccessControlPolicy &user_acl,
    const RGWAccessControlPolicy &bucket_acl,
    const boost::optional<Policy> &bucket_policy,
    const vector<Policy> &identity_policies,
...
static inline bool
check_deferred_bucket_only_acl(const DoutPrefixProvider *dpp,
                               struct perm_state_base *const s,
                               const RGWAccessControlPolicy &user_acl,
                               const RGWAccessControlPolicy &bucket_acl,
                               const uint8_t deferred_check, const int perm) {
  return (
      s->defer_to_bucket_acls == deferred_check &&
...
/*
  ...
  user_acl, bucket_acl, object_acl: 사용자, 버킷, 객체에 대한
  RGWAccessControlPolicy 접근 제어 정책. bucket_policy: 버킷에 대한 선택적인
  Policy. identity_policies, session_policies: 정체성 및 세션 관련 정책들의
  벡터. op: 수행하려는 작업을 나타내는 uint64_t 값.
...
bool verify_object_permission(const DoutPrefixProvider *dpp,
  ...
  auto identity_policy_res = eval_identity_or_session_policies(
      dpp, identity_policies, s->env, op, ARN(obj));
  if (identity_policy_res == Effect::Deny)
    return false;

  rgw::IAM::PolicyPrincipal princ_type = rgw::IAM::PolicyPrincipal::Other;
  auto r = eval_or_pass(dpp, bucket_policy, s->env, *s->identity, op, ARN(obj),
                        princ_type);
  if (r == Effect::Deny)
    return false;
  ...
  if (!session_policies.empty()) {
    auto session_policy_res = eval_identity_or_session_policies(
        dpp, session_policies, s->env, op, ARN(obj));
    if (session_policy_res == Effect::Deny) {
      return false;
    }
    if (princ_type == rgw::IAM::PolicyPrincipal::Role) {
  ...
  if (check_deferred_bucket_perms(dpp, s, obj.bucket, user_acl, bucket_acl,
                                  bucket_policy, identity_policies,
                                  session_policies,
                                  RGW_DEFER_TO_BUCKET_ACLS_RECURSE, op) ||
      check_deferred_bucket_perms(
  ...
  bool ret = object_acl.verify_permission(
      dpp, *s->identity, s->perm_mask, perm, nullptr, /* http_referrer */
      s->bucket_access_conf && s->bucket_access_conf->ignore_public_acls());
  if (ret) {
    return true;
  }
  ...
  /* we already verified the user mask above, so we pass swift_perm as the mask
     here, otherwise the mask might not cover the swift permissions bits */
...
bool verify_object_permission(const DoutPrefixProvider *dpp, req_state *const s,
                              ...
                              const uint64_t op) {
  perm_state_from_req_state ps(s);
  return verify_object_permission(dpp, &ps, obj, user_acl, bucket_acl,
                                  object_acl, bucket_policy, identity_policies,
                                  session_policies, op);
}
...
bool verify_object_permission_no_policy(
    ...
    const RGWAccessControlPolicy &user_acl,
    const RGWAccessControlPolicy &bucket_acl,
    const RGWAccessControlPolicy &object_acl, const int perm) {
  if (check_deferred_bucket_only_acl(dpp, s, user_acl, bucket_acl,
  ...
  bool ret = object_acl.verify_permission(
      dpp, *s->identity, s->perm_mask, perm, nullptr, /* http referrer */
      s->bucket_access_conf && s->bucket_access_conf->ignore_public_acls());
  if (ret) {
    return true;
  }
...
bool verify_object_permission_no_policy(const DoutPrefixProvider *dpp,
  ...
  return verify_object_permission_no_policy(dpp, &ps, s->user_acl,
                                            s->bucket_acl, s->object_acl, perm);
}
...
bool verify_object_permission(const DoutPrefixProvider *dpp, req_state *s,
  ...
  return verify_object_permission(
      dpp, &ps, rgw_obj(s->bucket->get_key(), s->object->get_key()),
      s->user_acl, s->bucket_acl, s->object_acl, s->iam_policy,
      s->iam_user_policies, s->session_policies, op);
}
...
void RGWUserCaps::dump(Formatter *f) const { dump(f, "caps"); }
...
bool RGWUserCaps::is_valid_cap_type(const string &tp) {
  static const char *cap_type[] = {
      "user",  "users",       "buckets",   "metadata",      "info",
      "usage", "zone",        "bilog",     "mdlog",         "datalog",
      "roles", "user-policy", "amz-cache", "oidc-provider", "ratelimit"};
...
bool match_policy(std::string_view pattern, std::string_view input,
  ...
  const uint32_t flag2 = flag & (MATCH_POLICY_ACTION | MATCH_POLICY_ARN)
                             ...
                             : 0;
  const bool colonblocks =
      !(flag & (MATCH_POLICY_RESOURCE | MATCH_POLICY_STRING));
...
void RGWBucketEnt::dump(Formatter *f) const {
  ...
  encode_json("mtime", ut, f); /* mtime / creation time discrepancy needed for
...
void RGWBucketInfo::decode_json(JSONObj *obj) {
  ...
  JSONDecoder::decode_json("num_shards",
                           layout.current_index.layout.normal.num_shards, obj);
  uint32_t hash_type;
...
static struct rgw_flags_desc op_type_flags[] = {{RGW_OP_TYPE_READ, "read"},
...
rgw_global_init(const std::map<std::string, std::string> *defaults,
  ...
  if ((config_store == "dbstore") || (config_store == "motr") ||
      (config_store == "daos")) {
