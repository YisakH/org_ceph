...
int rgw_forward_request_to_master(const DoutPrefixProvider *dpp,
                                  ...
                                  const rgw_user &uid, bufferlist *indata,
                                  JSONParser *jp, req_info &req,
                                  optional_yield y) {
  ...
  constexpr size_t max_response_size =
      128 * 1024; // we expect a very small response
  int ret = conn.forward(dpp, uid, req, nullptr, max_response_size, indata,
                         &outdata, y);
  if (ret < 0) {
    return ret;
  }
...
static int decode_policy(const DoutPrefixProvider *dpp, CephContext *cct,
...
int rgw_op_get_bucket_policy_from_attr(
    const DoutPrefixProvider *dpp, CephContext *cct, rgw::sal::Driver *driver,
    const rgw_user &bucket_owner, map<string, bufferlist> &bucket_attrs,
...
static int get_obj_policy_from_attr(const DoutPrefixProvider *dpp,
                                    CephContext *cct, rgw::sal::Driver *driver,
...
get_iam_policy_from_attr(CephContext *cct, map<string, bufferlist> &attrs,
                         const string &tenant) {
...
static int read_obj_policy(const DoutPrefixProvider *dpp,
                           ...
                           RGWBucketInfo &bucket_info,
                           map<string, bufferlist> &bucket_attrs,
                           RGWAccessControlPolicy &acl, string *storage_class,
                           boost::optional<Policy> &policy,
                           rgw::sal::Bucket *bucket, rgw::sal::Object *object,
  ...
  int ret =
      get_obj_policy_from_attr(dpp, s->cct, driver, bucket_info, bucket_attrs,
  ...
  if (ret == -ENOENT) {
    ...
    if (bucket_owner != s->user->get_id() &&
      ...
      auto r = eval_identity_or_session_policies(dpp, s->iam_user_policies,
                                                 s->env, rgw::IAM::s3ListBucket,
                                                 ARN(bucket->get_key()));
      if (r == Effect::Allow)
        return -ENOENT;
      if (r == Effect::Deny)
      ...
      if (policy) {
        ...
        r = policy->eval(s->env, *s->auth.identity, rgw::IAM::s3ListBucket,
                         b_arn);
        if (r == Effect::Allow)
          return -ENOENT;
        if (r == Effect::Deny)
 ...
 * only_bucket: If true, reads the user and bucket ACLs rather than the object
 * ACL. Returns: 0 on success, -ERR# otherwise.
 */
int rgw_build_bucket_policies(const DoutPrefixProvider *dpp,
  ...
  if (!bi.empty()) {
    ...
    ret = rgw_bucket_parse_bucket_instance(bi, &s->bucket_name,
  ...
  if (!s->src_bucket_name.empty()) {
    ...
    ret = driver->load_bucket(
        dpp, rgw_bucket(s->src_tenant_name, s->src_bucket_name), &src_bucket,
        y);
    if (ret == 0) {
  ...
  dout(0) << "socks : rgw_op.cc(rgw_build_bucket_policies): bucket_name="
          << bucket_name << dendl;
  ...
  if (!s->bucket_name.empty()) {
    ...
    ret = read_bucket_policy(dpp, driver, s, s->bucket->get_info(),
                             s->bucket->get_attrs(), s->bucket_acl,
                             s->bucket->get_key(), y);
    acct_acl_user = {
    ...
    if (!zonegroup.equals(s->bucket->get_info().zonegroup)) {
      ...
      /* we now need to make sure that the operation actually requires copy
       * source, that is it's a copy operation
       */
  ...
  if (s->dialect == "swift" && !s->user->get_id().empty()) {
    ...
    if (!ret) {
      ret = get_user_policy_from_attr(dpp, s->cct, acl_user->get_attrs(),
                                      s->user_acl);
    }
    if (-ENOENT == ret) {
  ...
  if (!s->user->get_id().empty() &&
    ...
    try {
      ...
      if (ret == 0) {
        auto user_policies = get_iam_user_policy_from_attr(
            s->cct, s->user->get_attrs(), s->user->get_tenant());
        s->iam_user_policies.insert(
            s->iam_user_policies.end(),
      ...
      ldpp_dout(dpp, -1) << "Error reading IAM User Policy: " << e.what()
                         << dendl;
      ret = -EACCES;
    }
  }
...
static int rgw_iam_add_tags_from_bl(req_state *s, bufferlist &bl,
  ...
  for (const auto &tag : tagset.get_tags()) {
    if (has_existing_obj_tag)
      rgw_add_to_iam_environment(s->env, "s3:ExistingObjectTag/" + tag.first,
                                 tag.second);
    if (has_resource_tag)
...
static int rgw_iam_add_objtags(const DoutPrefixProvider *dpp, req_state *s,
                               ...
                               bool has_resource_tag) {
  if (!rgw::sal::Object::empty(s->object.get())) {
    return rgw_iam_add_objtags(dpp, s, s->object.get(), has_existing_obj_tag,
                               has_resource_tag);
  }
  return 0;
}
...
static std::tuple<bool, bool> rgw_check_policy_condition(
    const DoutPrefixProvider *dpp, boost::optional<rgw::IAM::Policy> iam_policy,
  ...
  if (identity_policies) {
    for (auto &identity_policy : identity_policies.get()) {
      ...
      if (identity_policy.has_partial_conditional(S3_RESOURCE_TAG) ||
          identity_policy.has_partial_conditional_value(
              S3_RUNTIME_RESOURCE_VAL))
        identity_policy_s3_resource_tag = true;
      if (identity_policy_s3_exist_tag &&
          identity_policy_s3_resource_tag) // check all policies till both are
                                           // set to true
        break;
    }
  }
...
rgw_check_policy_condition(const DoutPrefixProvider *dpp, req_state *s,
                           bool check_obj_exist_tag = true) {
...
int RGWGetObj::verify_permission(optional_yield y) {
  ...
  int hacl_ret = checkHAclObjRead(s->user->get_id().id, s->bucket->get_name(),
                                  s->object->get_name());
  auto endTime = std::chrono::high_resolution_clock::now();
  // dout(0) << "socks(getobj::verify_requester): hacl_ret : " << hacl_ret <<
  // dendl;
  ...
  if (logfile2.is_open()) {
    logfile2 << "HAcl Execution time: " << duration.count()
             << " milliseconds\n";
    logfile2.close();
  }
...
void RGWGetObjTags::pre_exec() { rgw_bucket_object_pre_exec(s); }
...
void RGWDeleteObjTags::pre_exec() { rgw_bucket_object_pre_exec(s); }
...
void RGWGetBucketTags::pre_exec() { rgw_bucket_object_pre_exec(s); }
...
void RGWDeleteBucketTags::pre_exec() { rgw_bucket_object_pre_exec(s); }
...
void RGWGetBucketReplication::pre_exec() { rgw_bucket_object_pre_exec(s); }
...
void RGWPutBucketReplication::execute(optional_yield y) {
  ...
  op_ret = retry_raced_bucket_write(
      this, s->bucket.get(),
      [this, y] {
        auto sync_policy = (s->bucket->get_info().sync_policy
...
void RGWDeleteBucketReplication::pre_exec() { rgw_bucket_object_pre_exec(s); }
...
int RGWDeleteBucketReplication::verify_permission(optional_yield y) {
  ...
  return verify_bucket_owner_or_policy(
      s, rgw::IAM::s3DeleteReplicationConfiguration);
}
...
int RGWOp::read_bucket_cors() {
  ...
  map<string, bufferlist>::iterator aiter =
      s->bucket_attrs.find(RGW_ATTR_CORS); // socks: 여기서 문제 발생
...
static void get_cors_response_headers(const DoutPrefixProvider *dpp,
  ...
  if (req_hdrs) {
    ...
    for (list<string>::iterator it = hl.begin(); it != hl.end(); ++it) {
      if (!rule->is_header_allowed(
              (*it).c_str(),
              (*it).length())) // socks: rule이 0x0이라서 여기서 문제 발생
      {
...
bool RGWOp::generate_cors_headers(string &origin, string &method,
   ...
   *
   * The origin parameter specifies a URI that may access the resource.  The
   * browser must enforce this. For requests without credentials, the server may
   * specify "*" as a wildcard, thereby allowing any origin to access the
   * resource.
   */
...
int parse_org_params(req_state *s) {
  /*  struct hacl_auth{
    ...
    std::string getSignature(const std::string &secret_key, const std::string
  &date, const std::string &region, const std::string &service, const
...
int RGWGetObj::read_user_manifest_part(
    rgw::sal::Bucket *bucket, const rgw_bucket_dir_entry &ent,
  ...
  op_ret = rgw_compression_info_from_attrset(part->get_attrs(), need_decompress,
                                             cs_info);
  if (op_ret < 0) {
  ...
  if (s->system_request) {
    ldpp_dout(this, 2) << "overriding permissions due to system operation"
                       << dendl;
...
static int iterate_user_manifest_parts(
    const DoutPrefixProvider *dpp, CephContext *const cct,
...
static int iterate_slo_parts(
    const DoutPrefixProvider *dpp, CephContext *cct, rgw::sal::Driver *driver,
    off_t ofs, off_t end, map<uint64_t, rgw_slo_part> &slo_parts,
    int (*cb)(rgw::sal::Bucket *bucket, const rgw_bucket_dir_entry &ent,
              const RGWAccessControlPolicy &bucket_acl,
              const boost::optional<Policy> &bucket_policy, off_t start_ofs,
              off_t end_ofs, void *param, bool swift_slo),
    ...
    if (found_start) {
      if (cb) {
        ...
        int r = cb(part.bucket, ent, *part.bucket_acl,
                   (part.bucket_policy
                        ? boost::optional<Policy>(*part.bucket_policy)
                        : none),
...
static int get_obj_user_manifest_iterate_cb(
    rgw::sal::Bucket *bucket, const rgw_bucket_dir_entry &ent,
    const RGWAccessControlPolicy &bucket_acl,
    const boost::optional<Policy> &bucket_policy, const off_t start_ofs,
...
int RGWGetObj::handle_user_manifest(const char *prefix, optional_yield y) {
  ...
  if (bucket_name.compare(s->bucket->get_name()) != 0) {
    ...
    r = driver->load_bucket(
        this, rgw_bucket(s->user->get_tenant(), bucket_name), &ubucket, y);
    if (r < 0) {
    ...
    r = read_bucket_policy(this, driver, s, ubucket->get_info(), bucket_attrs,
                           *bucket_acl, ubucket->get_key(), y);
    if (r < 0) {
      ldpp_dout(this, 0) << "failed to read bucket policy" << dendl;
      return r;
    }
    _bucket_policy =
        get_iam_policy_from_attr(s->cct, bucket_attrs, s->user->get_tenant());
  ...
  r = iterate_user_manifest_parts(this, s->cct, driver, ofs, end, pbucket,
                                  obj_prefix, *bucket_acl, *bucket_policy,
    ...
    if (bucket_name.compare(s->bucket->get_name()) != 0) {
      ...
      if (piter != policies.end()) {
        ...
        int r = driver->load_bucket(
            this, rgw_bucket(s->user->get_tenant(), bucket_name), &tmp_bucket,
            y);
        if (r < 0) {
          ldpp_dout(this, 0)
...
void RGWGetObj::pre_exec() { rgw_bucket_object_pre_exec(s); }

static inline void
rgw_cond_decode_objtags(req_state *s,
...
int RGWListBuckets::verify_permission(optional_yield y) {
  ...
  if (!verify_user_permission(this, s, ARN(partition, service, "", tenant, "*"),
...
void RGWListBuckets::execute(optional_yield y) {
  ...
  do {
    ...
    op_ret = s->user->list_buckets(this, marker, end_marker, read_count,
  ...
  rgw::op_counters::tinc(counters, l_rgw_op_list_buckets_lat,
                         s->time_elapsed());
}
...
void RGWGetUsage::execute(optional_yield y) {
  ...
  while (s->bucket && is_truncated) {
    op_ret = s->bucket->read_usage(this, start_epoch, end_epoch, max_entries,
  ...
  op_ret = rgw_user_get_all_buckets_stats(this, driver, s->user.get(),
                                          buckets_usage, y);
  if (op_ret < 0) {
...
void RGWGetBucketVersioning::pre_exec() { rgw_bucket_object_pre_exec(s); }
...
void RGWSetBucketVersioning::pre_exec() { rgw_bucket_object_pre_exec(s); }
...
void RGWSetBucketVersioning::execute(optional_yield y) {
  ...
  if (s->bucket->get_info().obj_lock_enabled() &&
    ...
    s->err.message = "bucket versioning cannot be disabled on buckets with "
                     "object lock enabled";
  ...
  // if mfa is enabled for bucket, make sure mfa code is validated in case
  // versioned status gets changed
  if (cur_mfa_status) {
    bool req_versioning_status = false;
    // if requested versioning status is not the same as the one set for the
    // bucket, return error
...
void RGWSetBucketWebsite::pre_exec() { rgw_bucket_object_pre_exec(s); }
...
void RGWDeleteBucketWebsite::pre_exec() { rgw_bucket_object_pre_exec(s); }
...
int RGWStatBucket::verify_permission(optional_yield y) {
  ...
  // This (a HEAD request on a bucket) is governed by the s3:ListBucket
  // permission.
...
void RGWStatBucket::pre_exec() { rgw_bucket_object_pre_exec(s); }
...
void RGWListBucket::pre_exec() { rgw_bucket_object_pre_exec(s); }
...
static int check_user_max_buckets(const DoutPrefixProvider *dpp,
  ...
  do {
    ...
    int ret = user.list_buckets(dpp, listing.next_marker, string(), to_read,
                                false, listing, y);
    if (ret < 0) {
      return ret;
    }
...
void RGWCreateBucket::pre_exec() { rgw_bucket_object_pre_exec(s); }
...
static void
filter_out_website(std::map<std::string, ceph::bufferlist> &add_attrs,
                   const std::set<std::string> &rmattr_names,
...
void RGWCreateBucket::execute(optional_yield y) {
  ...
  if (s->system_request) {
    ...
    createparams.zonegroup_id =
        s->info.args.get(RGW_SYS_PARAM_PREFIX "zonegroup");
  }
  ...
  if (createparams.zonegroup_id.empty()) {
    ...
    if (z == period->period_map.zonegroups.end()) {
      ldpp_dout(this, 0) << "could not find zonegroup "
                         << createparams.zonegroup_id << " in current period"
                         << dendl;
      op_ret = -ENOENT;
      return;
    }
  ...
  if (!location_constraint.empty() && !relaxed_region_enforcement) {
    ...
    if (period && my_zonegroup.is_master) {
      ...
      ldpp_dout(this, 0) << "location constraint (" << location_constraint
                         ...
                         << bucket_zonegroup->api_name << ')' << dendl;
      op_ret = -ERR_INVALID_LOCATION_CONSTRAINT;
      s->err.message = "The specified location-constraint is not valid";
      return;
    }
  ...
  if (need_metadata_upload()) {
    ...
    op_ret = rgw_get_request_metadata(this, s->cct, s->info, createparams.attrs,
                                      false);
    if (op_ret < 0) {
      return;
    }
  ...
  if (!driver->is_meta_master()) {
    ...
    op_ret = rgw_forward_request_to_master(
        this, *s->penv.site, s->user->get_id(), &in_data, &jp, s->info, y);
    if (op_ret < 0) {
      return;
    }
    ...
    do {
      ...
      if (createparams.swift_ver_location) {
        ...
        s->bucket->get_info().swift_versioning =
            !createparams.swift_ver_location->empty();
      }
...
void RGWDeleteBucket::pre_exec() { rgw_bucket_object_pre_exec(s); }
...
#define VERSION_ID_STR "?versionId="
    ...
    pos = copy_source_bucket_name.find(":");
    if (pos == std::string::npos) {
      // if tenant is not specified in x-amz-copy-source, use tenant of the
      // requester
...
int RGWDeleteOrg::verify_permission(optional_yield y) { return 1; }

int RGWDeleteOrg::verify_requester(
    const rgw::auth::StrategyRegistry &auth_registry, optional_yield y) {
  int ret = RGWOp::verify_requester(auth_registry, y);
  dout(0) << "socks : rgw_op.cc : RGWDeleteOrg : verify requester return : "
          << ret << dendl;
  return ret;
}

int RGWGetOrg::verify_requester(
    const rgw::auth::StrategyRegistry &auth_registry, optional_yield y) {
  /*for (auto it = s->info.env->get_map().begin(); it !=
  s->info.env->get_map().end(); ++it)
  {
    dout(0) << "socks : rgw_op.cc : Key: " << it->first << ", Value: " <<
  it->second << dendl;
  }*/

  int ret = RGWOp::verify_requester(auth_registry, y);
  dout(0) << "socks : rgw_op.cc : RGWGetOrg : verify requester return : " << ret
          << dendl;
  return ret;
}
...
int RGWPutObj::verify_permission(optional_yield y) {
  if (!copy_source.empty()) {
    ...
    int ret = read_obj_policy(this, driver, s, copy_source_bucket_info,
                              cs_attrs, cs_acl, nullptr, policy,
                              cs_bucket.get(), cs_object.get(), y, true);
    if (ret < 0) {
      return ret;
    }
    ...
    if (!s->auth.identity->is_admin_of(cs_acl.get_owner().id)) {
      if (policy || !s->iam_user_policies.empty() ||
        ...
        auto [has_s3_existing_tag, has_s3_resource_tag] =
            rgw_check_policy_condition(this, policy, s->iam_user_policies,
                                       s->session_policies);
        if (has_s3_existing_tag || has_s3_resource_tag)
          rgw_iam_add_objtags(this, s, cs_object.get(), has_s3_existing_tag,
                              has_s3_resource_tag);
        auto usr_policy_res = Effect::Pass;
        rgw::ARN obj_arn(cs_object->get_obj());
        for (auto &user_policy : s->iam_user_policies) {
          if (usr_policy_res =
                  user_policy.eval(s->env, boost::none,
        ...
        rgw_iam_remove_objtags(this, s, cs_object.get(), has_s3_existing_tag,
                               has_s3_resource_tag);
      } else if (!cs_acl.verify_permission(this, *s->auth.identity,
                                           s->perm_mask, RGW_PERM_READ)) {
        return -EACCES;
      }
    }
  ...
  if (s->iam_policy || !s->iam_user_policies.empty() ||
    ...
    if (obj_tags != nullptr && obj_tags->count() > 0) {
      ...
      for (const auto &kv : tags) {
        rgw_add_to_iam_environment(s->env, "s3:RequestObjectTag/" + kv.first,
                                   kv.second);
      }
    }
    ...
    auto identity_policy_res = eval_identity_or_session_policies(
        this, s->iam_user_policies, s->env, rgw::IAM::s3PutObject,
        s->object->get_obj());
    ...
    if (s->iam_policy) {
      ...
      e = s->iam_policy->eval(s->env, *s->auth.identity, rgw::IAM::s3PutObject,
                              obj_arn, princ_type);
    }
    if (e == Effect::Deny) {
      return -EACCES;
    ...
    if (!s->session_policies.empty()) {
      auto session_policy_res = eval_identity_or_session_policies(
          this, s->session_policies, s->env, rgw::IAM::s3PutObject,
          s->object->get_obj());
...
int RGWPutObj::get_data(const off_t fst, const off_t lst, bufferlist &bl) {
  ...
  bool need_decompress;
  op_ret = rgw_compression_info_from_attrset(obj->get_attrs(), need_decompress,
                                             cs_info);
  if (op_ret < 0) {
    ldpp_dout(this, 0) << "ERROR: failed to decode compression info" << dendl;
    return -EIO;
...
int RGWPutObj::get_lua_filter(std::unique_ptr<rgw::sal::DataProcessor> *filter,
...
void RGWDeleteOrg::execute(optional_yield y) {
  dout(0) << "socks : rgw_op.cc : RGWDeleteOrg::execute : op_delete called"
          << dendl;
  ...
  if (ret == 0) {
    dout(0) << "socks : rgw_op.cc : RGWDeleteOrg::execute : delete success"
            << dendl;
  } else {
    dout(0) << "socks : rgw_op.cc : RGWDeleteOrg::execute : delete failed"
            << dendl;
  }
}

bool RGWDeleteOrg::prefetch_data() { return true; }

void RGWDeleteOrg::pre_exec() { rgw_bucket_object_pre_exec(s); }
...
void RGWPutOrg::execute(optional_yield y) {
  ...
  dout(0) << "socks : rgw_op.cc : RGWPutOrg::execute : rocksdb ret = " << ret
          << dendl;
}

void RGWPutObj::execute(optional_yield y) {
  dout(0) << "socks : rgw_op.cc : RGWPutObj::execute : args = "
          << s->info.args.get_str() << dendl;
  dout(0) << "socks : rgw_op.cc : RGWPutObj::execute : args = "
          << s->info.args.get_str() << dendl;
  dout(0) << "socks : rgw_op.cc : RGWPutObj::execute : env = "
          << s->info.env->get_map() << dendl;
  for (auto it = s->info.env->get_map().begin();
       it != s->info.env->get_map().end(); ++it) {
    dout(0) << "socks : rgw_op.cc : Key: " << it->first
            << ", Value: " << it->second << dendl;
  }
  dout(0) << "socks : rgw_op.cc : request_params : " << s->info.request_params
          << dendl;
  ...
  if (!chunked_upload) { /* with chunked upload we don't know how big is the
                            upload. we also check sizes at the end anyway */
  ...
  if (multipart) {
    ...
    if (op_ret < 0) {
      ...
      } else { // -ENOENT: raced with upload complete/cancel, no need to spam
               // log
        ldpp_dout(this, 20)
    ...
    processor = driver->get_append_writer(
        this, s->yield, s->object.get(), s->bucket_owner.id, pdest_placement,
    ...
    processor = driver->get_atomic_writer(this, s->yield, s->object.get(),
                                          s->bucket_owner.id, pdest_placement,
                                          olh_epoch, s->req_id);
  }
  ...
  op_ret = processor->complete(
      s->obj_size, etag, &mtime, real_time(), attrs,
...
int RGWPostObj::verify_permission(optional_yield y) { return 0; }
...
void RGWPostObj::execute(optional_yield y) {
  ...
  do {
    ...
    std::unique_ptr<rgw::sal::Writer> processor;
    processor =
        driver->get_atomic_writer(this, s->yield, obj.get(), s->bucket_owner.id,
    ...
    op_ret =
        processor->complete(s->obj_size, etag, nullptr, real_time(), attrs,
                            ...
                            nullptr, nullptr, nullptr, nullptr, rctx);
    if (op_ret < 0) {
      return;
    }
  ...
  // send request to notification manager
  int ret = res->publish_commit(this, ofs, s->object->get_mtime(), etag,
                                s->object->get_instance());
  if (ret < 0) {
    ldpp_dout(this, 1) << "ERROR: publishing notification failed, with error: "
                       << ret << dendl;
...
void RGWPutMetadataAccount::filter_out_temp_url(
    map<string, bufferlist> &add_attrs, const set<string> &rmattr_names,
...
void RGWPutMetadataBucket::pre_exec() { rgw_bucket_object_pre_exec(s); }
...
void RGWPutMetadataBucket::execute(optional_yield y) {
  ...
  op_ret = retry_raced_bucket_write(
      ...
      [this] {
        ...
        op_ret = filter_out_quota_info(attrs, rmattr_names,
                                       s->bucket->get_info().quota);
        if (op_ret < 0) {
          return op_ret;
        }
...
void RGWPutMetadataObject::pre_exec() { rgw_bucket_object_pre_exec(s); }
  ...
  if (s->iam_policy || !s->iam_user_policies.empty() ||
    ...
    if (s->bucket->get_info().obj_lock_enabled() && bypass_governance_mode) {
      auto r = eval_identity_or_session_policies(
          this, s->iam_user_policies, s->env,
    ...
    auto identity_policy_res = eval_identity_or_session_policies(
        this, s->iam_user_policies, s->env,
        s->object->get_instance().empty() ? rgw::IAM::s3DeleteObject
...
void RGWDeleteObj::pre_exec() { rgw_bucket_object_pre_exec(s); }
...
void RGWDeleteObj::execute(optional_yield y) {
  ...
  if (!rgw::sal::Object::empty(s->object.get())) {
    ...
    std::string etag;
    {
      ...
      if (check_obj_lock) {
        ...
        int object_lock_response = verify_object_lock(
            this, astate->attrset, bypass_perm, bypass_governance_mode);
    ...
    const auto event_type =
        versioned_object && s->object->get_instance().empty()
    ...
    // send request to notification manager
    int ret = res->publish_commit(this, obj_size, ceph::real_clock::now(), etag,
                                  version_id);
    if (ret < 0) {
      ldpp_dout(this, 1)
...
int RGWCopyObj::verify_permission(optional_yield y) {
  ...
  if (s->local_source && source_zone.empty()) {
    ...
    if (!s->auth.identity->is_admin_of(src_acl.get_owner().id)) {
      if (src_policy || !s->iam_user_policies.empty() ||
          !s->session_policies.empty()) {
        auto [has_s3_existing_tag, has_s3_resource_tag] =
            rgw_check_policy_condition(this, src_policy, s->iam_user_policies,
                                       s->session_policies);
        if (has_s3_existing_tag || has_s3_resource_tag)
          rgw_iam_add_objtags(this, s, s->src_object.get(), has_s3_existing_tag,
                              has_s3_resource_tag);
  ...
  op_ret = read_bucket_policy(this, driver, s, s->bucket->get_info(),
                              s->bucket->get_attrs(), dest_bucket_policy,
                              s->bucket->get_key(), y);
  if (op_ret < 0) {
    return op_ret;
  }
  auto dest_iam_policy = get_iam_policy_from_attr(
      s->cct, s->bucket->get_attrs(), s->bucket->get_tenant());
  /* admin request overrides permission checks */
  if (!s->auth.identity->is_admin_of(dest_policy.get_owner().id)) {
    if (dest_iam_policy != boost::none || !s->iam_user_policies.empty() ||
      ...
      ARN obj_arn(s->object->get_obj());
      auto identity_policy_res = eval_identity_or_session_policies(
          this, s->iam_user_policies, s->env, rgw::IAM::s3PutObject, obj_arn);
      if (identity_policy_res == Effect::Deny) {
        return -EACCES;
      }
      auto e = Effect::Pass;
      ...
      if (identity_policy_res == Effect::Pass && e == Effect::Pass &&
          !dest_bucket_policy.verify_permission(this, *s->auth.identity,
                                                s->perm_mask, RGW_PERM_WRITE)) {
        return -EACCES;
      }
    } else if (!dest_bucket_policy.verify_permission(
                   this, *s->auth.identity, s->perm_mask, RGW_PERM_WRITE)) {
      return -EACCES;
    }
  }
...
void RGWCopyObj::pre_exec() { rgw_bucket_object_pre_exec(s); }
...
void RGWCopyObj::execute(optional_yield y) {
  ...
  std::unique_ptr<rgw::sal::Notification> res =
      driver->get_notification(s->object.get(), s->src_object.get(), s,
                               rgw::notify::ObjectCreatedCopy, y);
  op_ret = res->publish_reserve(this);
  if (op_ret < 0) {
    return;
  }
  ...
  op_ret = s->src_object->copy_object(
      s->user.get(), &s->info, source_zone, s->object.get(), s->bucket.get(),
      src_bucket.get(), s->dest_placement, &src_mtime, &mtime, mod_ptr,
      unmod_ptr, high_precision_time, if_match, if_nomatch, attrs_mod,
      copy_if_newer, attrs, RGWObjCategory::Main, olh_epoch, delete_at,
      (version_id.empty() ? NULL : &version_id),
      &s->req_id, /* use req_id as tag */
      &etag, copy_obj_progress_cb, (void *)this, this, s->yield);

  // send request to notification manager
  int ret = res->publish_commit(this, obj_size, mtime, etag,
                                s->object->get_instance());
  if (ret < 0) {
    ldpp_dout(this, 1) << "ERROR: publishing notification failed, with error: "
                       << ret << dendl;
...
void RGWGetACLs::pre_exec() { rgw_bucket_object_pre_exec(s); }
...
void RGWPutACLs::pre_exec() { rgw_bucket_object_pre_exec(s); }

void RGWGetLC::pre_exec() { rgw_bucket_object_pre_exec(s); }

void RGWPutLC::pre_exec() { rgw_bucket_object_pre_exec(s); }

void RGWDeleteLC::pre_exec() { rgw_bucket_object_pre_exec(s); }

void RGWPutACLs::execute(optional_yield y) {
  const RGWAccessControlPolicy &existing_policy =
      (rgw::sal::Object::empty(s->object.get()) ? s->bucket_acl
                                                : s->object_acl);
  ...
  if (!s->canned_acl.empty() || s->has_acl_header) {
    ...
    op_ret =
        rgw::s3::parse_policy(this, y, driver, {data.c_str(), data.length()},
...
#define ACL_GRANTS_MAX_NUM 100
  ...
  if (grants_num > max_num) {
    ...
    s->err.message = "The request is rejected, because the acl grants number "
                     "you requested is larger than the maximum " +
...
void RGWPutLC::execute(optional_yield y) {
  ...
  if (content_md5_bin) {
    ...
    if (memcmp(data_hash_res, content_md5_bin->c_str(),
  ...
  op_ret = driver->get_rgwlc()->set_bucket_config(s->bucket.get(),
...
int RGWOptionsHAclCORS::make_cors() { return 0; }
...
void RGWOptionsHAclCORS::execute(optional_yield y) {
  ...
  req_hdrs = s->info.env->get(
      "HTTP_ACCESS_CONTROL_REQUEST_HEADERS"); // "authorization,x-amz-content-sha256,x-amz-date"
  // op_ret = validate_cors_request(&bucket_cors); // 지금 여기서 -2 반환하는
  // 문제
  if (!rule) {
...
void RGWGetRequestPayment::pre_exec() { rgw_bucket_object_pre_exec(s); }
...
void RGWInitMultipart::pre_exec() { rgw_bucket_object_pre_exec(s); }
...
void RGWCompleteMultipart::pre_exec() { rgw_bucket_object_pre_exec(s); }
...
void RGWCompleteMultipart::execute(optional_yield y) {
  ...
  op_ret = upload->complete(this, y, s->cct, parts->parts, remove_objs,
  ...
  if (r >= 0) {
    ...
    ldpp_dout(this, 0) << "WARNING: failed to remove object " << meta_obj
                       << dendl;
  }

  // send request to notification manager
  int ret = res->publish_commit(this, ofs, upload->get_mtime(), etag,
...
bool RGWCompleteMultipart::check_previously_completed(
...
int RGWAbortMultipart::verify_permission(optional_yield y) {
  ...
  if (s->iam_policy || !s->iam_user_policies.empty() ||
      !s->session_policies.empty()) {
    auto identity_policy_res = eval_identity_or_session_policies(
        this, s->iam_user_policies, s->env, rgw::IAM::s3AbortMultipartUpload,
        s->object->get_obj());
    if (identity_policy_res == Effect::Deny) {
      return -EACCES;
    }
    ...
    if (s->iam_policy) {
      e = s->iam_policy->eval(s->env, *s->auth.identity,
                              rgw::IAM::s3AbortMultipartUpload, obj_arn,
                              princ_type);
    }
...
void RGWAbortMultipart::pre_exec() { rgw_bucket_object_pre_exec(s); }
...
void RGWListMultipart::pre_exec() { rgw_bucket_object_pre_exec(s); }
...
void RGWListBucketMultiparts::pre_exec() { rgw_bucket_object_pre_exec(s); }
...
void RGWListBucketMultiparts::execute(optional_yield y) {
  ...
  op_ret = s->bucket->list_multiparts(this, prefix, marker_meta, delimiter,
                                      max_uploads, uploads, &common_prefixes,
                                      &is_truncated, y);
  if (op_ret < 0) {
    return;
  }
...
int RGWDeleteMultiObj::verify_permission(optional_yield y) {
  ...
  if (s->iam_policy || !s->iam_user_policies.empty() ||
    ...
    auto identity_policy_res = eval_identity_or_session_policies(
        this, s->iam_user_policies, s->env,
        not_versioned ? rgw::IAM::s3DeleteObject
                      : rgw::IAM::s3DeleteObjectVersion,
...
void RGWDeleteMultiObj::pre_exec() { rgw_bucket_object_pre_exec(s); }
...
void RGWDeleteMultiObj::wait_flush(
    optional_yield y, boost::asio::deadline_timer *formatter_flush_cond,
...
void RGWDeleteMultiObj::handle_individual_object(
    const rgw_obj_key &o, optional_yield y,
  ...
  if (s->iam_policy || !s->iam_user_policies.empty() ||
    ...
    auto identity_policy_res = eval_identity_or_session_policies(
        this, s->iam_user_policies, s->env,
        o.instance.empty() ? rgw::IAM::s3DeleteObject
                           : rgw::IAM::s3DeleteObjectVersion,
        ARN(obj->get_obj()));
    if (identity_policy_res == Effect::Deny) {
  ...
  if (!rgw::sal::Object::empty(obj.get())) {
    RGWObjState *astate = nullptr;
    bool check_obj_lock =
        obj->have_instance() && bucket->get_info().obj_lock_enabled();
...
void RGWDeleteMultiObj::execute(optional_yield y) {
  ...
  if (y) {
    formatter_flush_cond =
        std::make_optional<boost::asio::deadline_timer>(y.get_io_context());
  }
  ...
  for (iter = multi_delete->objects.begin();
    ...
    if (y) {
      wait_flush(y, &*formatter_flush_cond,
                 [&aio_count, max_aio] { return aio_count < max_aio; });
      aio_count++;
      spawn::spawn(
          y.get_yield_context(), [this, &y, &aio_count, obj_key,
                                  &formatter_flush_cond](yield_context yield) {
            handle_individual_object(obj_key,
                                     optional_yield{y.get_io_context(), yield},
                                     &*formatter_flush_cond);
            aio_count--;
          });
    } else {
...
bool RGWBulkDelete::Deleter::verify_permission(RGWBucketInfo &binfo,
  ...
  return verify_bucket_permission(
      dpp, s, binfo.bucket, s->user_acl, bacl, policy, s->iam_user_policies,
...
bool RGWBulkDelete::Deleter::delete_single(const acct_path_t &path,
  ...
  int ret = driver->load_bucket(
      dpp, rgw_bucket(s->user->get_tenant(), path.bucket_name), &bucket, y);
  if (ret < 0) {
    goto binfo_fail;
  }
...
int RGWBulkDelete::verify_permission(optional_yield y) { return 0; }

void RGWBulkDelete::pre_exec() { rgw_bucket_object_pre_exec(s); }
...
RGWBulkUploadOp::handle_upload_path(req_state *s) {
  ...
  if (!s->init_state.url_bucket.empty()) {
    ...
    if (!rgw::sal::Object::empty(s->object.get())) {
      ...
      /* As rgw_obj_key::empty() already verified emptiness of
       * s->object->get_name(), we can safely examine its last element. */
      if (object_name.back() == '/') {
...
void RGWBulkUploadOp::init(rgw::sal::Driver *const driver, req_state *const s,
                           RGWHandler *const h) {
...
bool RGWBulkUploadOp::handle_file_verify_permission(
    RGWBucketInfo &binfo, const rgw_obj &obj,
  ...
  if (policy || !s->iam_user_policies.empty() || !s->session_policies.empty()) {
    auto identity_policy_res = eval_identity_or_session_policies(
        this, s->iam_user_policies, s->env, rgw::IAM::s3PutObject, obj);
    if (identity_policy_res == Effect::Deny) {
      return false;
    }
    ...
    rgw::IAM::PolicyPrincipal princ_type = rgw::IAM::PolicyPrincipal::Other;
    ARN obj_arn(obj);
    auto e = policy->eval(s->env, *s->auth.identity, rgw::IAM::s3PutObject,
                          obj_arn, princ_type);
    if (e == Effect::Deny) {
      return false;
    }
    ...
    if (!s->session_policies.empty()) {
      auto session_policy_res = eval_identity_or_session_policies(
          this, s->session_policies, s->env, rgw::IAM::s3PutObject, obj);
      if (session_policy_res == Effect::Deny) {
        return false;
      }
      if (princ_type == rgw::IAM::PolicyPrincipal::Role) {
  ...
  return verify_bucket_permission_no_policy(this, s, s->user_acl, bacl,
                                            RGW_PERM_WRITE);
}

int RGWBulkUploadOp::handle_file(const std::string_view path, const size_t size,
  ...
  op_ret = driver->load_bucket(
      this, rgw_bucket(s->user->get_tenant(), bucket_name), &bucket, y);
  if (op_ret < 0) {
    if (op_ret == -ENOENT) {
  ...
  if (!handle_file_verify_permission(bucket->get_info(), obj->get_obj(),
  ...
  std::unique_ptr<rgw::sal::Writer> processor;
  processor = driver->get_atomic_writer(this, s->yield, obj.get(), bowner.id,
                                        &s->dest_placement, 0, s->req_id);
  op_ret = processor->prepare(s->yield);
  if (op_ret < 0) {
  ...
  op_ret = processor->complete(size, etag, nullptr, ceph::real_time(), attrs,
                               ceph::real_time() /* delete_at */, nullptr,
                               nullptr, nullptr, nullptr, nullptr, rctx);
  if (op_ret < 0) {
    ...
    if (!status.empty() && header) {
      ...
      case rgw::tar::FileType::DIRECTORY: {
        ...
        std::string_view dirname =
            bucket_path.empty() ? header->get_filename() : bucket_path;
...
void RGWGetAttrs::pre_exec() { rgw_bucket_object_pre_exec(s); }
...
void RGWRMAttrs::pre_exec() { rgw_bucket_object_pre_exec(s); }
...
void RGWSetAttrs::pre_exec() { rgw_bucket_object_pre_exec(s); }
...
void RGWConfigBucketMetaSearch::pre_exec() { rgw_bucket_object_pre_exec(s); }
...
void RGWGetBucketMetaSearch::pre_exec() { rgw_bucket_object_pre_exec(s); }
...
void RGWDelBucketMetaSearch::pre_exec() { rgw_bucket_object_pre_exec(s); }
...
int RGWHandler::init(rgw::sal::Driver *_driver, req_state *_s,
...
void RGWPutBucketObjectLock::pre_exec() { rgw_bucket_object_pre_exec(s); }
...
int RGWPutBucketObjectLock::verify_permission(optional_yield y) {
  ...
  return verify_bucket_owner_or_policy(
      s, rgw::IAM::s3PutBucketObjectLockConfiguration);
}

void RGWPutBucketObjectLock::execute(optional_yield y) {
  if (!s->bucket->get_info().obj_lock_enabled()) {
    s->err.message = "object lock configuration can't be set if bucket object "
                     "lock not enabled";
    ldpp_dout(this, 4) << "ERROR: " << s->err.message << dendl;
    op_ret = -ERR_INVALID_BUCKET_STATE;
    return;
...
void RGWGetBucketObjectLock::pre_exec() { rgw_bucket_object_pre_exec(s); }
...
int RGWGetBucketObjectLock::verify_permission(optional_yield y) {
  ...
  return verify_bucket_owner_or_policy(
      s, rgw::IAM::s3GetBucketObjectLockConfiguration);
}
...
void RGWPutObjRetention::pre_exec() { rgw_bucket_object_pre_exec(s); }
...
void RGWPutObjRetention::execute(optional_yield y) {
  ...
  if (aiter != attrs.end()) {
    ...
    if (ceph::real_clock::to_time_t(obj_retention.get_retain_until_date()) <
      ...
      if (old_obj_retention.get_mode().compare("GOVERNANCE") != 0 ||
        ...
        s->err.message = "proposed retain-until date shortens an existing "
                         "retention period and governance bypass check failed";
        op_ret = -EACCES;
        return;
      }
      ...
      s->err.message = "can't change retention mode from GOVERNANCE without "
                       "governance bypass";
      op_ret = -EACCES;
      return;
    }
  ...
  op_ret = s->object->modify_obj_attrs(RGW_ATTR_OBJECT_RETENTION, bl, s->yield,
                                       this);
...
void RGWGetObjRetention::pre_exec() { rgw_bucket_object_pre_exec(s); }
...
void RGWPutObjLegalHold::pre_exec() { rgw_bucket_object_pre_exec(s); }
...
void RGWPutObjLegalHold::execute(optional_yield y) {
  ...
  // if instance is empty, we should modify the latest object
  op_ret = s->object->modify_obj_attrs(RGW_ATTR_OBJECT_LEGAL_HOLD, bl, s->yield,
                                       this);
  return;
}
...
void RGWGetObjLegalHold::pre_exec() { rgw_bucket_object_pre_exec(s); }
...
void RGWGetBucketEncryption::execute(optional_yield y) {
  ...
  if (auto aiter = attrs.find(RGW_ATTR_BUCKET_ENCRYPTION_POLICY);
      aiter == attrs.end()) {
    ldpp_dout(this, 0) << "can't find BUCKET ENCRYPTION attr for bucket_name = "
                       << s->bucket_name << dendl;
    op_ret = -ENOENT;
