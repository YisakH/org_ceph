...
int RGWGetObj::parse_range(void)
{
  ...
  { // RFC2616 suffix-byte-range-spec
    ofs = -end;
    end = -1;
  }
...
int rgw_build_bucket_policies(const DoutPrefixProvider *dpp, rgw::sal::Driver *driver, req_state *s, optional_yield y)
{
  ...
  if (!s->src_bucket_name.empty())
  {
    ...
    ret = driver->load_bucket(dpp, rgw_bucket(s->src_tenant_name, s->src_bucket_name),
                              &src_bucket, y);
    if (ret == 0)
    {
  ...
  if (!s->bucket_name.empty())
  {
     ...
     * overwritten. */
    ret = driver->load_bucket(dpp, rgw_bucket(s->bucket_tenant, s->bucket_name, s->bucket_instance_id),
                              &s->bucket, y);
    if (ret < 0)
    {
      if (ret != -ENOENT)
  ...
  if (attr_iter != attrs.end())
  {
    ...
    try
    {
      ...
      if (m.get_tier_type() == "cloud-s3")
      {
        ...
        { // fetch object for sync and set cloud_tier attrs
          bufferlist t, t_tier;
...
{
  auto iam_action = s->object->get_instance().empty() ? rgw::IAM::s3GetObjectTagging : rgw::IAM::s3GetObjectVersionTagging;
...
{
  auto iam_action = s->object->get_instance().empty() ? rgw::IAM::s3PutObjectTagging : rgw::IAM::s3PutObjectVersionTagging;
...
int RGWDeleteObjTags::verify_permission(optional_yield y)
{
  if (!rgw::sal::Object::empty(s->object.get()))
  {
    auto iam_action = s->object->get_instance().empty() ? rgw::IAM::s3DeleteObjectTagging : rgw::IAM::s3DeleteObjectVersionTagging;
  ...
  for (const auto &entry : slo_info.entries)
  {
    ...
    if (bucket_name.compare(s->bucket->get_name()) != 0)
    {
      ...
      else
      {
        ...
        int r = driver->load_bucket(this, rgw_bucket(s->user->get_tenant(), bucket_name),
                                    &tmp_bucket, y);
        if (r < 0)
        {
          ldpp_dout(this, 0) << "could not get bucket info for bucket="
...
#define VERSION_ID_STR "?versionId="
    ...
    ret = driver->load_bucket(this, rgw_bucket(copy_source_tenant_name, copy_source_bucket_name),
                              &bucket, y);
    if (ret < 0)
    {
...
int RGWPutObj::verify_permission(optional_yield y)
{
  ...
  if (s->iam_policy || !s->iam_user_policies.empty() || !s->session_policies.empty())
  {
    ...
    if (!s->session_policies.empty())
    {
      ...
      else if (princ_type == rgw::IAM::PolicyPrincipal::Other)
...
void RGWPutObj::execute(optional_yield y)
{
  ...
  if (multipart)
  {
    ...
    if (op_ret < 0)
    {
      ...
      { // -ENOENT: raced with upload complete/cancel, no need to spam log
...
void RGWPostObj::execute(optional_yield y)
{
  ...
  // make reservation for notification if needed
  std::unique_ptr<rgw::sal::Notification> res = driver->get_notification(s->object.get(), s->src_object.get(), s, rgw::notify::ObjectCreatedPost, y);
...
void RGWDeleteObj::execute(optional_yield y)
{
  ...
  if (!rgw::sal::Object::empty(s->object.get()))
  {
    ...
    std::unique_ptr<rgw::sal::Notification> res = driver->get_notification(s->object.get(), s->src_object.get(), s,
                                                                           event_type, y);
    op_ret = res->publish_reserve(this);
    if (op_ret < 0)
    {
...
int RGWCopyObj::init_processing(optional_yield y)
{
  ...
  op_ret = driver->load_bucket(this, rgw_bucket(s->src_tenant_name, s->src_bucket_name),
                               &src_bucket, y);
  if (op_ret < 0)
  {
    if (op_ret == -ENOENT)
...
int RGWGetACLs::verify_permission(optional_yield y)
{
  ...
  if (!rgw::sal::Object::empty(s->object.get()))
  {
    auto iam_action = s->object->get_instance().empty() ? rgw::IAM::s3GetObjectAcl : rgw::IAM::s3GetObjectVersionAcl;
    if (has_s3_existing_tag || has_s3_resource_tag)
      rgw_iam_add_objtags(this, s, has_s3_existing_tag, has_s3_resource_tag);
...
#define ACL_GRANTS_MAX_NUM 100
  ...
  if (grants_num > max_num)
  {
    ...
    s->err.message = "The request is rejected, because the acl grants number you requested is larger than the maximum " + std::to_string(max_num) + " grants allowed in an acl.";
    return;
  }
...
void RGWCompleteMultipart::execute(optional_yield y)
{
  ...
  // make reservation for notification if needed
  std::unique_ptr<rgw::sal::Notification> res = driver->get_notification(meta_obj.get(), nullptr, s, rgw::notify::ObjectCreatedCompleteMultipartUpload, y, &s->object->get_name());
  op_ret = res->publish_reserve(this);
  if (op_ret < 0)
  {
    return;
                                                 ...
                                                 boost::asio::deadline_timer *formatter_flush_cond)
{
  ...
  // make reservation for notification if needed
  const auto versioned_object = s->bucket->versioning_enabled();
  const auto event_type = versioned_object && obj->get_instance().empty() ? rgw::notify::ObjectRemovedDeleteMarkerCreated : rgw::notify::ObjectRemovedDelete;
  std::unique_ptr<rgw::sal::Notification> res = driver->get_notification(obj.get(), s->src_object.get(), s, event_type, y);
  op_ret = res->publish_reserve(this);
  if (op_ret < 0)
  {
...
bool RGWBulkDelete::Deleter::delete_single(const acct_path_t &path, optional_yield y)
{
  ...
  int ret = driver->load_bucket(dpp, rgw_bucket(s->user->get_tenant(), path.bucket_name),
                                &bucket, y);
  if (ret < 0)
  {
    goto binfo_fail;
                                 ...
                                 AlignedStreamGetter &body, optional_yield y)
{
  ...
  op_ret = driver->load_bucket(this, rgw_bucket(s->user->get_tenant(), bucket_name),
                               &bucket, y);
  if (op_ret < 0)
  {
    if (op_ret == -ENOENT)
...
void RGWPutBucketPolicy::execute(optional_yield y)
{
  ...
  try
  {
    ...
    op_ret = retry_raced_bucket_write(
        this, s->bucket.get(), [&p, this, &attrs]
        {
...
void RGWPutBucketEncryption::execute(optional_yield y)
{
  ...
  op_ret = retry_raced_bucket_write(
      this, s->bucket.get(), [this, y, &conf_bl]
      {
    rgw::sal::Attrs attrs = s->bucket->get_attrs();
