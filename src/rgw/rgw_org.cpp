...
bool OrgPermissionFlags::operator<=(const OrgPermissionFlags &other) const {
  return (get <= other.get) && (put <= other.put) && (gra <= other.gra) &&
         (del <= other.del);
}

bool OrgPermissionFlags::operator<(const OrgPermissionFlags &other) const {
  bool isStrictlyLess = false; // 진부분집합 여부를 판단하기 위한 변수
  if ((!other.get || get) && (!other.put || put) && (!other.gra || gra) &&
      (!other.del || del)) {
...
std::string trimSlashes(const std::string &str) {
  ...
  if (start == std::string::npos)
    return ""; // String consists only of slashes

  size_t end = str.find_last_not_of('/');
  return str.substr(start, end - start + 1);
}

std::string getObjectPath(const std::string &bucket_name,
...
int DBManager::getAllPartialMatchData(
...
int toRGWOrg(const std::string &key, const std::string &value, RGWOrg *rgwOrg) {
  ...
  try {
    ...
    std::getline(iss2, token2, ' ');
    rgwOrg->orgPermissionFlags->get =
        std::stoi(token2) != 0; // 문자열을 bool로 변환
    std::getline(iss2, token2, ' ');
    rgwOrg->orgPermissionFlags->put =
        std::stoi(token2) != 0; // 문자열을 bool로 변환
    std::getline(iss2, token2, ' ');
    rgwOrg->orgPermissionFlags->del =
        std::stoi(token2) != 0; // 문자열을 bool로 변환
    std::getline(iss2, token2, ' ');
    rgwOrg->orgPermissionFlags->gra =
        std::stoi(token2) != 0; // 문자열을 bool로 변환
...
int RGWOrg::putRGWOrg() { // 여기서 orgPermission이 그냥 기본으로 들어오는 문제
                          // 발생.
  std::string key = user + ":" + orgPermissionFlags->path;
  std::string value = authorizer + " " + std::to_string(tier) + " " +
                      std::to_string(orgPermissionFlags->get) + " " +
                      std::to_string(orgPermissionFlags->put) + " " +
                      std::to_string(orgPermissionFlags->del) + " " +
                      std::to_string(orgPermissionFlags->gra);
...
int RGWOrg::getPartialMatchRgwOrg(
    const std::string &user, const std::string &path,
    RGWOrg *rgwOrg) { // TODO: 조금 더 효율적인 방법으로 수정 필요. 예를 들어
                      // ret를 설정하지 말고 바로 return 해도 됨
...
int putAcl(const std::string &user, const std::string &path,
           const std::string &authorizer, int tier, bool get, bool put,
           bool del, bool gra) {
  if (user == authorizer) {
  ...
  auto orgPermission =
      std::make_unique<OrgPermissionFlags>(get, put, del, gra, path);
...
int putAcl(RGWOrg &rgwOrg) {
  return putAcl(rgwOrg.getUser(), rgwOrg.getOrgPermission()->path,
                rgwOrg.getAuthorizer(), rgwOrg.getTier(),
                rgwOrg.getOrgPermission()->get, rgwOrg.getOrgPermission()->put,
                rgwOrg.getOrgPermission()->del, rgwOrg.getOrgPermission()->gra);
}

int deleteAcl(const std::string &request_user, const std::string &user,
...
int RGWOrgDec::appendDecEdge(const std::string &user,
                             const std::vector<std::string> &dec_list) {
  ...
  if (ret == -RGW_HBAC_KEY_NOT_FOUND) {
    ...
    for (const auto &dec : dec_list) {
      ...
      if (std::find(existing_dec_list.begin(), existing_dec_list.end(), dec) ==
          existing_dec_list.end()) {
...
std::string RGWOrg::toString() {
  return "user: " + user + ", authorizer: " + authorizer +
         ", tier: " + std::to_string(tier) +
         ", get: " + std::to_string(orgPermissionFlags->get) +
         ", put: " + std::to_string(orgPermissionFlags->put) +
         ", del: " + std::to_string(orgPermissionFlags->del) +
         ", gra: " + std::to_string(orgPermissionFlags->gra) +
         ", path: " + orgPermissionFlags->path;
};

int RGWOrgDec::decListToString(std::vector<std::string> &dec_list,
...
int RGWOrgDec::getDec(const std::string &user,
...
int checkAclRead(const std::string &request_user,
...
int checkAclWrite(const std::string &request_user,
                  const std::string &target_user, const std::string &path,
                  const std::string &authorizer, int tier, bool get, bool put,
                  bool del, bool gra) {
  int request_user_tier = -1, target_user_tier = -1;
  int ret = -1;
  ...
  if (request_user_org == nullptr ||
      !request_user_org->getOrgPermission()->gra) { // grant 권한이 없는 경우
  ...
  if (rgwOrg != nullptr) {
    ...
    if (ancPermission != nullptr &&
        orgPermission <
            *ancPermission) { // anc의 권한이 요청한 권한을 포함하지 못하는 경우
      return -RGW_ORG_PERMISSION_NOT_ALLOWED;
    }
...
int checkHAclObjWrite(const std::string &request_user,
                      const std::string &bucket_name,
                      const std::string &object_name) {
  const std::string path = getObjectPath(bucket_name, object_name);
  RGWOrg *rgwOrg = getAcl(request_user, path, false);
...
std::string getSignature(const std::string &secret_key, const std::string &date,
...
std::string generateCanonicalHeaders(const std::string &hostHeader,
...
std::string generatePayloadHash(const std::string &payload) {
  return sha256_hex(
      payload); // If the payload is empty, sha256_hex("") will be called.
}

std::string getAuthHeader(const std::string &access_key,
  ...
  std::string signature =
      getSignature(secret_key, date, region, service, string_to_sign);

  return createAuthHeader(access_key, date, region, service, signedHeaders,
                          signature);
}

std::string createAuthHeader(const std::string &accessKey,
  ...
  // 인증 헤더 구성
  std::string authHeader = "AWS4-HMAC-SHA256 Credential=" + credential +
...
int RGWOrgUser::putUser(std::string user, std::string anc,
                        std::vector<std::string> dec_list) {
  int ret = -1;
  ...
  if (dec_list.size() > 0) {
    // TODO: dec가 이미 존재하는 경우에 존재하는 dec를 찾고 user의 자손이 되도록
    // 설정해야 할 필요 있음
...
int RGWOrgUser::deleteUserRelation(const std::string &user,
                                   const std::vector<std::string> &dec_list) {
  std::vector<std::string> existing_dec_list;
  ...
  for (auto dec : dec_list) {
    auto it =
        std::find(existing_dec_list.begin(), existing_dec_list.end(), dec);
    if (it != existing_dec_list.end()) {
      existing_dec_list.erase(it);
    }
  }
...
int RGWOrgUser::deleteUser(const std::string &user) {
  ...
  if (anc_ret == -RGW_HBAC_KEY_NOT_FOUND &&
      dec_ret == -RGW_HBAC_KEY_NOT_FOUND) {
    return deleteOnlyUser(user);
  }
  if (anc_ret == -RGW_HBAC_KEY_NOT_FOUND) {
    return deleteWithDescendants(user, dec_list);
  }
  if (dec_ret == -RGW_HBAC_KEY_NOT_FOUND) {
...
int RGWOrgUser::deleteWithDescendants(
    const std::string &user, const std::vector<std::string> &dec_list) {
  for (const auto &dec : dec_list) {
...
int RGWOrgUser::deleteWithBoth(const std::string &user, const std::string &anc,
                               const std::vector<std::string> &dec_list) {
  for (const auto &dec : dec_list) {
...
int RGWOrgTier::updateUserTier(const std::string &start_user) {
  ...
  for (auto dec : dec_list) {
    ...
    if (ret < 0 && ret != RGW_HBAC_KEY_NOT_FOUND) {
      return ret;
    }
  }
  return 0;
...
bool validateRGWOrgPermission(std::string user, std::string path, bool get,
                              bool put, bool del, bool gra) {
  ...
  // if request user has more permission than input get, put, del, gra, return
  // true

  if ((get && !orgPermission->get) || (put && !orgPermission->put) ||
      (del && !orgPermission->del) || (gra && !orgPermission->gra)) {
    return false;
  }
...
int AclDB::getAllPartialMatchAcl(
    const std::string &prefix,
...
int AclDB::getSuperPathsForPrefix(
    const std::string &userPrefix,
    std::vector<std::pair<std::string, RGWOrg>> &values) {
  std::istringstream iss(userPrefix);
  std::string segment;
  std::string accumulatedPath;
  std::string userPathPrefix = userPrefix.substr(
      0, userPrefix.find(":") + 1); // 사용자 이름 추출 (예: "user3:")
  ...
  return values.empty() ? RGW_HBAC_KEY_NOT_FOUND : 0;
}

int RGWOrgDec::getRGWOrgDecTree(const std::string &start_user,
                                nlohmann::json &j) {
  ...
  while (!q.empty()) {
    ...
    nlohmann::json cur_j = {{"name", cur_name},
                            {"id", id++},
    ...
    for (auto &dec : dec_list) {
      ...
      nlohmann::json child_ref = {{"name", dec},
                                  {"id", id++},
                                  {"children", nlohmann::json::array()},
                                  {"permission", nlohmann::json::array()}};
      cur_j["children"].push_back(
          child_ref); // 자식 노드 참조를 children에 추가
...
// 유저가 해당 권한이 있는지 검사
int checkAclPermission(const std::string &request_user, const bool get,
                       const bool put, const bool del, const bool gra,
                       const std::string &path) {
