...
#define RGW_HBAC_KEY_NOT_FOUND 30004
...
public:
  ...
  OrgPermissionFlags(bool get, bool put, bool del, bool gra)
      : get(get), put(put), del(del), gra(gra) {}
  OrgPermissionFlags(bool get, bool put, bool del, bool gra, std::string path)
...
public:
  ...
  DBManager(const std::string &dbPath)
      : dbPath(std::move(dbPath)), db(nullptr) {
    // if(dbName == "RocksDB"){
  ...
  rocksdb::Status getStatus() const { return status; }
  int getData(const std::string &key, std::string &value);
  int getAllPartialMatchData(
...
public:
  ...
  int getAllPartialMatchAcl(
      const std::string &prefix,
      std::vector<std::pair<std::string, RGWOrg>> &values);
  static int
  getSuperPathsForPrefix(const std::string &prefix,
                         std::vector<std::pair<std::string, RGWOrg>> &values);
...
public:
  ...
  RGWOrg(std::string user, const std::string &authorizer, uint16_t tier,
      ...
      : user(std::move(user)), authorizer(authorizer), tier(tier),
        orgPermissionFlags(orgPermission) {}
  ...
  const std::string &getAuthorizer() const { return authorizer; }

  uint16_t getTier() const { return tier; }

  OrgPermissionFlags *getOrgPermission() const { return orgPermissionFlags; }

  void setUser(const std::string &user) { RGWOrg::user = user; }

  void setAuthorizer(const std::string &authorizer) {
    RGWOrg::authorizer = authorizer;
  }

  void setTier(int tier) { RGWOrg::tier = tier; }
  ...
  static int getPartialMatchRgwOrg(const std::string &user,
                                   const std::string &path, RGWOrg *rgwOrg);
};
...
class RGWOrgDec {
public:
  static int getDec(const std::string &user,
  ...
  static int decListToString(std::vector<std::string> &dec_list,
                             std::string *dec_list_str);
  static int appendDecEdge(const std::string &user, const std::string &dec);
  static int appendDecEdge(const std::string &user,
...
public:
  static int
  putUser(std::string user, std::string anc = "",
          std::vector<std::string> dec_list = std::vector<std::string>());
  static int putUser(std::string user, std::string anc,
                     std::string dec_list_str);
  static int deleteUser(const std::string &user);
  static int deleteUserRelation(const std::string &user,
                                const std::vector<std::string> &dec_list);

  static int deleteOnlyUser(const std::string &user);
  static int deleteWithDescendants(const std::string &user,
                                   const std::vector<std::string> &dec_list);
  static int deleteWithAncestor(const std::string &user);
  static int deleteWithBoth(const std::string &user, const std::string &anc,
                            const std::vector<std::string> &dec_list);
};
...
int checkAclWrite(const std::string &request_user, const std::string &user,
                  const std::string &path, const std::string &authorizer,
                  int tier, bool get, bool put, bool del, bool gra);
...
bool validateRGWOrgPermission(std::string user, std::string path,
                              bool get = false, bool put = false,
                              bool del = false, bool gra = false);
