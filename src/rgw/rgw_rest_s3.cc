...
static int create_s3_policy(req_state *s, rgw::sal::Driver *driver,
  ...
  if (s->has_acl_header) {
    ...
    return rgw::s3::create_policy_from_headers(s, driver, owner, *s->info.env,
                                               policy);
  }
...
void RGWOptionsHAclCORS_ObjStore_S3::send_response() {
  ...
  // get_response_params(hdrs, exp_hdrs, &max_age); // socks: 여기서 rules를
  // 가져오지 못해 에러 발생

  dout(0) << "socks : RGWOptionsHAclCORS_ObjStore_S3::send_response() origin="
          << origin << " req_meth=" << req_meth << " hdrs=" << hdrs
          << " exp_hdrs=" << exp_hdrs << " max_age=" << max_age << dendl;
  const char *req_headers =
      s->info.env->get("HTTP_ACCESS_CONTROL_REQUEST_HEADERS");

  dout(0)
      << "socks : RGWOptionsHAclCORS_ObjStore_S3::send_response() req_headers="
      << req_headers << dendl;
...
RGWOp *RGWHandler_REST_Org_S3::op_options() {
  dout(0) << "socks : rgw_rest_s3.cc : op_options called" << dendl;
  return new RGWOptionsHAclCORS_ObjStore_S3;
}
...
discover_aws_flavour(const req_info &info, const req_state *const s) {
  ...
  const char *http_auth =
      (!tmp_auth || !tmp_auth[0]) ? s->hacl_auth.auth_header.c_str() : tmp_auth;
...
void update_attribute_map(const std::string &input, AttributeMap &map) {
  ...
  if (pos != std::string::npos) {
    ...
    const auto key_or_value_rhs =
        url_decode(key_or_value.substr(pos + 1, key_or_value.size() - 1));
...
RGWRESTMgr_S3::get_handler(rgw::sal::Driver *driver, req_state *const s,
  ...
  ldpp_dout(s, 0) << "socks: bucket handler ㄹ호출될 때 decoded uri: "
                  << s->decoded_uri << dendl;
  return new RGWHandler_REST_Bucket_S3(auth_registry, enable_pubsub);
}
...
AWSEngine::VersionAbstractor::auth_data_t AWSGeneralAbstractor::get_auth_data(
    const req_state *const s) const // 요것도 실행됨: socks
{
  AwsVersion version;
  AwsRoute route;
    ...
    if (!is_v4_payload_streamed(exp_payload_hash)) {
      ...
      const auto cmpl_factory =
          std::bind(AWSv4ComplMulti::create, s, date, credential_scope,
                    client_signature, std::placeholders::_1);
      return {access_key_id, client_signature,
              session_token, std::move(string_to_sign),
              sig_factory,   cmpl_factory};
    }
...
rgw::auth::Engine::result_t rgw::auth::s3::LocalEngine::authenticate(
  ...
  auto apl = apl_factory->create_apl_local(cct, s, user->get_info(), k.subuser,
                                           std::nullopt, access_key_id);
