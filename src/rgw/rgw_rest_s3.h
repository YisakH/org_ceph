...
public:
  RGWListBucket_ObjStore_S3() : objs_container(false) { default_max = 1000; }
...
public:
  ...
  void send_partial_response(
      const rgw_obj_key &key, bool delete_marker,
...
public:
  ...
  public:
    ...
    using signature_factory_t = std::function<server_signature_t(
        CephContext *cct, const std::string &secret_key,
...
public:
  explicit AWSGeneralAbstractor(CephContext *const cct) : cct(cct) {}
...
public:
  ...
  using signature_factory_t = std::function<signature_headers_t(
      const DoutPrefixProvider *dpp, const std::string &secret_key,
...
protected:
  ...
  result_t authenticate(const DoutPrefixProvider *dpp,
                        ...
                        const completer_factory_t &completer_factory,
                        const req_state *s, optional_yield y) const override;

public:
  LDAPEngine(CephContext *const cct, rgw::sal::Driver *driver,
             const VersionAbstractor &ver_abstractor,
             const rgw::auth::RemoteApplier::Factory *const apl_factory)
...
public:
  LocalEngine(CephContext *const cct, rgw::sal::Driver *driver,
              const VersionAbstractor &ver_abstractor,
              const rgw::auth::LocalApplier::Factory *const apl_factory)
      : AWSEngine(cct, ver_abstractor), driver(driver),
        apl_factory(apl_factory) {}
...
public:
  STSEngine(CephContext *const cct, rgw::sal::Driver *driver,
            const VersionAbstractor &ver_abstractor,
            const rgw::auth::LocalApplier::Factory *const local_apl_factory,
            const rgw::auth::RemoteApplier::Factory *const remote_apl_factory,
            const rgw::auth::RoleApplier::Factory *const role_apl_factory)
