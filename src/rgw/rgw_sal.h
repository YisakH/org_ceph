 ...
 *
 * The Store Abstraction Layer is an API that separates the top layer of RGW
 * that handles client protocols (such as S3 or Swift) from the bottom layer of
 * RGW that interacts with a backing store.  It allows the creation of multiple
 * backing stores that can co-exist with a single RGW instance, and allows the
 * creation of stacking layers of translators that can modify operations as they
 * pass down the stack. Examples of translators might be a cache layer, a
 * duplication layer that copies operations to multiple stores, or a policy
 * layer that sends some operations to one store and some to another.
 *
 * The basic unit of a SAL implementation is the Store.  Whether an actual
 * backing store or a translator, there will be a Store implementation that
 * represents it.  Examples are the RadosStore that communicates via RADOS with
 * a Ceph cluster, and the DBStore that uses a SQL db (such as SQLite3) as a
 * backing store.  There is a singleton instance of each Store.
 *
 * Data within RGW is owned by a User.  The User is the unit of authentication
 * and access control.
 *
 * Data within RGW is stored as an Object.  Each Object is a single chunk of
 * data, owned by a single User, contained within a single Bucket.  It has
 * metadata associated with it, such as size, owner, and so on, and a set of
 * key-value attributes that can contain anything needed by the top half.
 *
 * Data with RGW is organized into Buckets.  Each Bucket is owned by a User, and
 * contains Objects.  There is a single, flat layer of Buckets, there is no
 * hierarchy, and each Object is contained in a single Bucket.
 *
 * Instantiations of SAL classes are done as unique pointers, using
 * std::unique_ptr. Instances of these classes are acquired via getters, and
 * it's up to the caller to manage the lifetime.
 *
 * @note Anything using RGWObjContext is subject to change, as that type will
 * not be used in the final API.
 * @{
 */
...
enum AttrsMod { ATTRSMOD_NONE = 0, ATTRSMOD_REPLACE = 1, ATTRSMOD_MERGE = 2 };
...
public:
   ...
   *
   * An empty bufferlist is given to request that any buffered data be flushed,
 ...
 *
 * The Driver is the base abstraction of the SAL layer.  It represents a base
 * storage mechanism, or a intermediate stacking layer.  There is a single
 * instance of a given Driver per RGW, and this Driver mediates all access to
 * it's backing.
 *
 * A Driver contains, loosely, @a User, @a Bucket, and @a Object entities.  The
 * @a Object contains data, and it's associated metadata.  The @a Bucket
 * contains Objects, and metadata about the bucket.  Both Buckets and Objects
 * are owned by a @a User, which is the basic unit of access control.
 *
 * A Driver also has metadata and some global responsibilities.  For example, a
 * driver is responsible for managing the LifeCycle activities for it's data.
 */
class Driver {
public:
  Driver() {}
  ...
  virtual int get_user_by_swift(const DoutPrefixProvider *dpp,
                                ...
                                std::unique_ptr<User> *user) = 0;
  /** Get a basic Object.  This Object is not looked up, and is incomplete,
   * since is does not have a bucket.  This should only be used when an Object
   * is needed before there is a Bucket, otherwise use the get_object() in the
   * Bucket class. */
  virtual std::unique_ptr<Object> get_object(const rgw_obj_key &k) = 0;
  /** Get a Bucket by info.  Does not query the driver, just uses the give
   * bucket info. */
  ...
  virtual int load_bucket(const DoutPrefixProvider *dpp, const rgw_bucket &b,
                          ...
                          optional_yield y) = 0;
  /** get hbac */
  virtual int load_hbac(const DoutPrefixProvider *dpp, rgw_hbac_info info,
  ...
  /** Get a @a Notification object.  Used to communicate with non-RGW daemons,
   * such as management/tracking software */
  /** RGWOp variant */
  ...
  virtual std::unique_ptr<Notification>
  get_notification(const DoutPrefixProvider *dpp, rgw::sal::Object *obj,
   ...
   */
  virtual int read_topics(const std::string &tenant, rgw_pubsub_topics &topics,
  ...
  /** Get access to the coroutine registry.  Used to create new coroutine
   * managers */
  virtual RGWCoroutinesManagerRegistry *get_cr_registry() = 0;

  /** Log usage data to the driver.  Usage data is things like bytes
  ...
  virtual int log_op(const DoutPrefixProvider *dpp, std::string &oid,
                     bufferlist &bl) = 0;
  /** Register this driver to the service map.  Somewhat Rados specific; may be
   * removed*/
  virtual int
  register_to_service_map(const DoutPrefixProvider *dpp,
                          const std::string &daemon_type,
                          const std::map<std::string, std::string> &meta) = 0;
  /** Get default quota info.  Used as fallback if a user or bucket has no quota
   * set*/
  ...
  virtual int set_buckets_enabled(const DoutPrefixProvider *dpp,
                                  ...
                                  bool enabled, optional_yield y) = 0;
  ...
  get_data_sync_manager(const rgw_zone_id &source_zone) = 0;
  ...
  virtual void wakeup_data_sync_shards(
      ...
      boost::container::flat_map<
          int, boost::container::flat_set<rgw_data_notify_entry>>
          &shard_ids) = 0;
  ...
  virtual int
  read_all_usage(const DoutPrefixProvider *dpp, uint64_t start_epoch,
  ...
  virtual int meta_remove(const DoutPrefixProvider *dpp,
                          std::string &metadata_key, optional_yield y) = 0;
  ...
  virtual std::unique_ptr<LuaManager>
  get_lua_manager(const std::string &luarocks_path) = 0;
  /** Get an IAM Role by name etc. */
  virtual std::unique_ptr<RGWRole>
  get_role(std::string name, std::string tenant, std::string path = "",
  ...
  virtual int
  get_oidc_providers(const DoutPrefixProvider *dpp, const std::string &tenant,
  ...
  get_compression_type(const rgw_placement_rule &rule) = 0;
 ...
 *
 * This represents a user.  In general, there will be a @a User associated with
 * an OP (the user performing the OP), and potentially several others acting as
 * owners. Lifetime of a User is a bit tricky , since it must last as long as
 * any Buckets associated with it.  A User has associated metadata, including a
 * set of key/value attributes, and statistics (including usage) about the User.
 */
class User {
public:
  User() {}
  ...
  /** Clone a copy of this user.  Used when modification is necessary of the
   * copy */
  ...
  virtual int merge_and_store_attrs(const DoutPrefixProvider *dpp,
                                    Attrs &new_attrs, optional_yield y) = 0;
  ...
  virtual int trim_usage(const DoutPrefixProvider *dpp, uint64_t start_epoch,
                         uint64_t end_epoch, optional_yield y) = 0;

  /** Load this User from the backing store.  requires ID to be set, fills all
   * other fields. */
  ...
  virtual int store_user(const DoutPrefixProvider *dpp, optional_yield y,
                         bool exclusive, RGWUserInfo *old_info = nullptr) = 0;
 ...
 *
 * This represents a bucket.  A bucket is a container for objects.  It is owned
 * by a user, and has it's own set of metadata, including a set of key/value
 * attributes.  A bucket may not contain other buckets, only objects.  Buckets
 * have Access Control Lists (ACLs) that control what users can access the
 * contents of the bucket, and in what ways.
 */
class Bucket {
public:
  /**
  ...
  virtual int remove_bypass_gc(int concurrent_max, bool keep_index_consistent,
                               optional_yield y,
                               const DoutPrefixProvider *dpp) = 0;
  ...
  virtual int set_acl(const DoutPrefixProvider *dpp,
                      RGWAccessControlPolicy &acl, optional_yield y) = 0;
  ...
  virtual int create(const DoutPrefixProvider *dpp, const CreateParams &params,
                     optional_yield y) = 0;
  ...
  /** Change the owner of this bucket in the backing store.  Current owner must
   * be set.  Does not change ownership of the objects in the bucket. */
  ...
  virtual int put_info(const DoutPrefixProvider *dpp, bool exclusive,
                       ceph::real_time mtime, optional_yield y) = 0;
  ...
  virtual int try_refresh_info(const DoutPrefixProvider *dpp,
                               ceph::real_time *pmtime, optional_yield y) = 0;
  ...
  virtual int trim_usage(const DoutPrefixProvider *dpp, uint64_t start_epoch,
                         uint64_t end_epoch, optional_yield y) = 0;
  /** Remove objects from the bucket index of this bucket.  May be removed from
   * API */
  virtual int
  remove_objs_from_index(const DoutPrefixProvider *dpp,
                         std::list<rgw_obj_index_key> &objs_to_unlink) = 0;
  /** Check the state of the bucket index, and get stats from it.  May be
   * removed from API */
  virtual int
  check_index(const DoutPrefixProvider *dpp,
  ...
  virtual int set_tag_timeout(const DoutPrefixProvider *dpp,
                              uint64_t timeout) = 0;
  /** Remove this specific bucket instance from the backing store.  May be
   * removed from API */
  ...
  /** Clone a copy of this bucket.  Used when modification is necessary of the
   * copy */
  ...
  virtual int
  list_multiparts(const DoutPrefixProvider *dpp, const std::string &prefix,
  ...
  /** Read the bucket notification config into @a notifications with and
   * (optionally) @a objv_tracker */
  virtual int read_topics(rgw_pubsub_bucket_topics &notifications,
                          RGWObjVersionTracker *objv_tracker, optional_yield y,
                          const DoutPrefixProvider *dpp) = 0;
  /** Write @a notifications with (optionally) @a objv_tracker into the bucket
   * notification config */
  virtual int write_topics(const rgw_pubsub_bucket_topics &notifications,
                           RGWObjVersionTracker *objv_tracker, optional_yield y,
                           const DoutPrefixProvider *dpp) = 0;
...
public:
  ...
  struct PermissionFlags{
    ...
    bool gra;
    std::string path;
    PermissionFlags(bool get, bool put, bool del, bool gra) : get(get),
  put(put), del(del), gra(gra) {} PermissionFlags(bool get, bool put, bool del,
  bool gra, std::string path) : get(get), put(put), del(del), gra(gra),
  path(path){} PermissionFlags() : get(false), put(false), del(false),
  gra(false) {} bool operator<=(const PermissionFlags &other) const; bool
  operator<(const PermissionFlags &other) const; }permission_flags;*/

  Hbac() = default;
  /*Hbac(const rgw_hbac_info& info) : user(info.user),
  authorizer(info.authorizer), path(info.path){ permission_flags.get =
  info.perms.get; permission_flags.put = info.perms.put; permission_flags.del =
  info.perms.del; permission_flags.gra = info.perms.gra;
  }**/
  virtual ~Hbac() = default;
  virtual int load_hbac(const DoutPrefixProvider *dpp, optional_yield y) {
    return 0;
  };
  virtual int store_hbac(const DoutPrefixProvider *dpp, optional_yield y) {
    return 0;
  };
 ...
 * represents a blob of data, a set of metadata (such as size, owner, ACLs,
 * etc.) and a set of key/value attributes.  Objects may be versioned.  If a
 * versioned object is written to, a new object with the same name but a
 * different version is created, and the old version of the object is still
 * accessible.  If an unversioned object is written to, it is replaced, and the
...
public:
   ...
   *
   * This represents a Read operation on an Object.  Read operations are
   * optionally asynchronous, using the iterate() API.
   */
  struct ReadOp {
    struct Params {
  ...
  virtual int copy_object(
      ...
      void *progress_data, const DoutPrefixProvider *dpp, optional_yield y) = 0;
  ...
  /** Check to see if this object has an empty key.  This means it's
   * uninitialized */
  virtual bool empty() const = 0;
  ...
  virtual int get_obj_state(const DoutPrefixProvider *dpp, RGWObjState **state,
                            optional_yield y, bool follow_olh = true) = 0;
  /** Set the object state for this object */
  virtual void set_obj_state(RGWObjState &_state) = 0;
  /** Set attributes for this object from the backing store.  Attrs can be set
   * or deleted.  @note the attribute APIs may be revisited in the future. */
  virtual int set_obj_attrs(const DoutPrefixProvider *dpp, Attrs *setattrs,
                            Attrs *delattrs, optional_yield y) = 0;
  ...
  virtual int delete_obj_attrs(const DoutPrefixProvider *dpp,
                               const char *attr_name, optional_yield y) = 0;
  ...
  virtual int swift_versioning_restore(bool &restored, /* out */
                                       const DoutPrefixProvider *dpp,
                                       optional_yield y) = 0;
  /** Copy the current version of a swift object to the configured destination
   * bucket*/
  ...
  virtual int get_torrent_info(const DoutPrefixProvider *dpp, optional_yield y,
                               bufferlist &bl) = 0;
 ...
 *
 * This represents a multipart upload.  For large objects, it's inefficient to
 * do a single, long-lived upload of the object.  Instead, protocols such as S3
 * allow the client to start a multipart upload, and then upload object in
 * smaller parts in parallel.  A MultipartUpload consists of a target bucket, a
 * unique identifier, and a set of upload parts.
 */
 ...
 *
 * Lifecycle processing loops over the objects in a bucket, applying per-bucket
 * policy to each object.  Examples of policy can be deleting after a certain
 * amount of time, deleting extra versions, changing the storage class, and so
 * on.
 */
class Lifecycle {
public:
  ...
  /** Single entry in a lifecycle run.  Multiple entries can exist processing
   * different buckets. */
  struct LCEntry {
    LCEntry() = default;
    ...
    friend inline std::ostream &operator<<(std::ostream &out,
                                           const std::unique_ptr<LCEntry> &p) {
      out << p.get();
      return out;
    }
  };
  ...
  get_serializer(const std::string &lock_name, const std::string &oid,
...
public:
  ...
  /** Indicate the successful completion of the event associated with this
   * notification */
 ...
 *
 * Writing is done through a set of filters.  This allows chaining filters to do
 * things like compression and encryption on async writes.  This is the base
 * abstraction for those filters.
 */
...
public:
  ...
  virtual void
  get_placement_target_names(std::set<std::string> &names) const = 0;
 ...
 *
 * This abstraction allows access to information about zones.  This can be the
 * zone containing the RGW, or another zone.
 */
class Zone {
public:
 ...
 * scripts; adding, deleting, and listing packages
 */
class LuaManager {
public:
  ...
  virtual int get_script(const DoutPrefixProvider *dpp, optional_yield y,
                         const std::string &key, std::string &script) = 0;
  ...
  virtual int put_script(const DoutPrefixProvider *dpp, optional_yield y,
                         const std::string &key, const std::string &script) = 0;
 ...
 *
 * This will manage the singleton instances of the various drivers.  Drivers
 * come in two varieties: Full and Raw.  A full driver is suitable for use in a
 * radosgw daemon.  It has full access to the cluster, if any.  A raw driver is
 * a stripped down driver, used for admin commands.
 */
class DriverManager {
public:
  struct Config {
  ...
  get_storage(const DoutPrefixProvider *dpp, CephContext *cct,
    ...
    rgw::sal::Driver *driver = init_storage_provider(
        dpp, cct, cfg, use_gc_thread, use_lc_thread, quota_threads,
        run_sync_thread, run_reshard_thread, run_notification_thread, use_cache,
        use_gc, y);
    return driver;
  }
  ...
  static rgw::sal::Driver *
  init_storage_provider(const DoutPrefixProvider *dpp, CephContext *cct,
                        const Config &cfg, bool use_gc_thread,
                        bool use_lc_thread, bool quota_threads,
                        bool run_sync_thread, bool run_reshard_thread,
                        bool run_notification_thread, bool use_metadata_cache,
                        bool use_gc, optional_yield y);
  /** Initialize a new raw Driver */
  static rgw::sal::Driver *
  init_raw_storage_provider(const DoutPrefixProvider *dpp, CephContext *cct,
                            const Config &cfg);
