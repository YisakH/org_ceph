...
int rgw_user_sync_all_stats(const DoutPrefixProvider *dpp,
  ...
  int ret = 0;
  do {
    ret = user->list_buckets(dpp, listing.next_marker, string(), max_entries,
                             false, listing, y);
    if (ret < 0) {
...
int rgw_user_get_all_buckets_stats(
    const DoutPrefixProvider *dpp, rgw::sal::Driver *driver,
    rgw::sal::User *user, map<string, bucket_meta_entry> &buckets_usage_map,
    optional_yield y) {
  size_t max_entries = dpp->get_cct()->_conf->rgw_list_buckets_max_chunk;
