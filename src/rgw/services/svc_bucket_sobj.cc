...
public:
  RGWSI_Bucket_SObj_Module(RGWSI_Bucket_SObj::Svc &_svc)
      : RGWSI_MBSObj_Handler_Module("bucket"), svc(_svc) {}
...
public:
  RGWSI_BucketInstance_SObj_Module(RGWSI_Bucket_SObj::Svc &_svc)
      : RGWSI_MBSObj_Handler_Module("bucket.instance"), svc(_svc),
  ...
  // 'tenant/' is used in bucket instance keys for sync to avoid parsing
  // ambiguity with the existing instance[:shard] format. once we parse the
  // shard, the / is replaced with a : to match the [tenant:]instance format
  ...
  /*
   * hash entry for mdlog placement. Use the same hash key we'd have for the
   * bucket entry point, so that the log entries end up at the same log shard,
   * so that we process them in order
   */
...
int RGWSI_Bucket_SObj::read_bucket_entrypoint_info(
    RGWSI_Bucket_EP_Ctx &ctx, const string &key,
  ...
  auto params = RGWSI_MBSObj_GetParams(&bl, pattrs, pmtime)
...
int RGWSI_Bucket_SObj::store_bucket_entrypoint_info(
    RGWSI_Bucket_EP_Ctx &ctx, const string &key, RGWBucketEntryPoint &info,
    bool exclusive, real_time mtime, const map<string, bufferlist> *pattrs,
...
int RGWSI_Bucket_SObj::remove_bucket_entrypoint_info(
    RGWSI_Bucket_EP_Ctx &ctx, const string &key,
    RGWObjVersionTracker *objv_tracker, optional_yield y,
    const DoutPrefixProvider *dpp) {
...
int RGWSI_Bucket_SObj::read_bucket_instance_info(
    RGWSI_Bucket_BI_Ctx &ctx, const string &key, RGWBucketInfo *info,
    real_time *pmtime, map<string, bufferlist> *pattrs, optional_yield y,
    const DoutPrefixProvider *dpp, rgw_cache_entry_info *cache_info,
    boost::optional<obj_version> refresh_version) {
  string cache_key("bi/");
  ...
  int ret = do_read_bucket_instance_info(ctx, key, &e.info, &e.mtime, &e.attrs,
...
int RGWSI_Bucket_SObj::do_read_bucket_instance_info(
    RGWSI_Bucket_BI_Ctx &ctx, const string &key, RGWBucketInfo *info,
...
int RGWSI_Bucket_SObj::read_bucket_info(
    RGWSI_Bucket_X_Ctx &ctx, const rgw_bucket &bucket, RGWBucketInfo *info,
    real_time *pmtime, map<string, bufferlist> *pattrs,
    boost::optional<obj_version> refresh_version, optional_yield y,
    const DoutPrefixProvider *dpp) {
  rgw_cache_entry_info cache_info;
  ...
  if (!bucket.bucket_id.empty()) {
    return read_bucket_instance_info(ctx.bi, get_bi_meta_key(bucket), info,
                                     pmtime, pattrs, y, dpp, &cache_info,
                                     refresh_version);
  }
  ...
  int ret = read_bucket_entrypoint_info(ctx.ep, bucket_entry, &entry_point, &ot,
  ...
  /* data is in the bucket instance object, we need to get attributes from
   * there, clear everything that we got
   */
  if (pattrs) {
    pattrs->clear();
  }
...
int RGWSI_Bucket_SObj::store_bucket_instance_info(
    RGWSI_Bucket_BI_Ctx &ctx, const string &key, RGWBucketInfo &info,
    std::optional<RGWBucketInfo *> orig_info, bool exclusive, real_time mtime,
    const map<string, bufferlist> *pattrs, optional_yield y,
    const DoutPrefixProvider *dpp) {
  // Encoding the info
  bufferlist bl;
  encode(info, bl);
  ...
  if (!orig_info && !exclusive) {
    int r = read_bucket_instance_info(ctx, key, &shared_bucket_info, nullptr,
                                      nullptr, y, dpp, nullptr, boost::none);
    if (r < 0) {
      if (r != -ENOENT) {
...
int RGWSI_Bucket_SObj::remove_bucket_instance_info(
    RGWSI_Bucket_BI_Ctx &ctx, const string &key, const RGWBucketInfo &info,
    RGWObjVersionTracker *objv_tracker, optional_yield y,
    const DoutPrefixProvider *dpp) {
  RGWSI_MBSObj_RemoveParams params;
  int ret =
  ...
  if (r < 0) {
    ldpp_dout(dpp, 0)
        ...
        << dendl;
    /* returning success as index is just keeping hints, so will keep extra
     * hints, but bucket removal succeeded
     */
  }
...
int RGWSI_Bucket_SObj::read_bucket_stats(const RGWBucketInfo &bucket_info,
                                         RGWBucketEnt *ent, optional_yield y,
                                         const DoutPrefixProvider *dpp) {
  ent->count = 0;
  ent->size = 0;
  ent->size_rounded = 0;
